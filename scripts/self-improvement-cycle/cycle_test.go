...
func cycleFixture() (options, inputs) {
	...
	in := inputs{
		...
		Plan:       document[planDocument]{Value: planDocument{SchemaVersion: "gooo/self-improvement-generation/v7", BaseSHA: base, HeadSHA: head, PlanDigest: digest, Decision: "FIXED_POINT", Reason: "EXACT_FIXED_POINT"}, FileSHA256: docHash},
		Execution:  document[executionDocument]{Value: executionDocument{SchemaVersion: "gooo/meta-operation-execution/v7", BaseSHA: base, HeadSHA: head, PlanDigest: digest, ManifestDigest: digest, IndicatorDecisionLedgerDigest: ledger, IndicatorDecisionLedgerCount: 1, Decision: "FIXED_POINT", Reason: "EXACT_FIXED_POINT"}, FileSHA256: docHash},
		Receipts:   document[receiptDocument]{Value: receiptDocument{SchemaVersion: "gooo/meta-operation-receipt-report/v3", BaseSHA: base, HeadSHA: head, PlanDigest: digest, ReportDigest: digest, IndicatorDecisionLedgerDigest: ledger, IndicatorDecisionLedgerCount: 1, Decision: "FIXED_POINT", Reason: "EXACT_FIXED_POINT"}, FileSHA256: docHash},
...
func contractFixture(head, digest string) contractDocument {
	...
	coverage := make([]contractCoverage, len(generation.DefaultRegistry()))
	for index := range coverage {
		coverage[index].Covered = true
	}
