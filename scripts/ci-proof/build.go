...
func verifyReceipt(filename string, bundle proofBundle) error {
	...
	if receipt.Schema != receiptSchema || receipt.Operation != "verify" || receipt.Relation != "conformance" || receipt.AllowedIntent != "verification-only" || receipt.BaseSHA != bundle.BaseSHA || receipt.HeadSHA != bundle.HeadSHA || receipt.Ref != bundle.Ref || receipt.EventRef != bundle.EventRef || receipt.CheckoutRef != bundle.CheckoutRef || receipt.RunID != bundle.RunID || receipt.RunAttempt != bundle.RunAttempt || receipt.WorkflowSHA != bundle.WorkflowSHA || receipt.Decision != bundle.Decision || len(receipt.Jobs) != len(proofJobs) || len(receipt.Artifacts) != len(bundle.Artifacts) || receipt.WriteEffect != "none" || receipt.Cache.Key != bundle.Cache.Key || receipt.Cache.Outcome != bundle.Cache.Outcome {
