...
The manifest also carries the machine-readable `catalog_path` field. It is
validated against this checked-in path, and the workflow summary prints the
manifest's value together with severity, scope, handoff status, and evidence
references so an agent can act without parsing a human-only default.
