...
const workflow = {workflow_ref: 'kimjooyoon/meta-ontology-go/.github/workflows/ci-guardian.yml@refs/heads/dev', workflow_sha: sha('e'), runtime_ref: 'refs/heads/dev', runtime_sha: sha('e'), check_name: 'CI guardian', check_run_id: 7, app_id: 15368};
const candidate = reconciliation.candidateFromPull({pull, changedFiles: [{filename: '.github/ci-governance.json', status: 'modified'}], sourceDevSHA: sha('f'), mergeBaseSHA: sha('g'), sourceDevTree: tree, candidateTree: tree, workflow});
...
test('UNKNOWN preserves the exact six-field evidence shape when dev moves', () => {
  const result = reconciliation.evaluate({route: reconciliation.ROUTE, candidate, live_main_sha: pull.base.sha, live_dev_sha: sha('h'), authorization});
