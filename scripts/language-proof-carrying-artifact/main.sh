...
jq -e '.consumer_receipt.output_exists == false and .consumer_receipt.target_digest == "" and .consumer_receipt.output_digest == "" and .artifact_use_authority == "" and ([.proofs[] | select(.phase == "PRELIMINARY" and .state == "OBSERVED" and .passed == true and .consumer_gate_open == false)] | length) == 1 and ([.proofs[] | select(.phase == "PRELIMINARY" and .state == "OPEN" and .passed == true and .consumer_gate_open == true)] | length) == 2' "$output/bundle-preliminary-report.json"

preliminary_negative_fixture_total=7
...
jq -e '([.proofs[] | select(.phase == "FINAL" and .state == "DISCHARGED" and .passed == true and .consumer_gate_open == false)] | length) == 3' "$output/bundle-report.json"
