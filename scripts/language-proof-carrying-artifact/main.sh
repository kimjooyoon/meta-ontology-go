...
# Policy fixtures are bundled before the content-addressed bundle is sealed so
# the complete verifyArtifact path can reconstruct each one by path and digest.
: > "$output/policy-missing.gooo"
sed 's|kind=EVIDENCE_DIGEST_MISMATCH;rank=4|kind=EVIDENCE_DIGEST_MISMATCH;rank=not-a-rank|' "$output/source.gooo" > "$output/policy-malformed-rank.gooo"
sed 's|kind=RECIPE_MISMATCH;rank=11|kind=RECIPE_MISMATCH;rank=10|' "$output/source.gooo" > "$output/policy-duplicate-rank.gooo"
cp "$output/source.gooo" "$output/policy-unknown-issue.gooo"
...
# This intervention changes source meaning and swaps two declared policy ranks.
# The selected issue is observed from a tampered artifact, so the policy
# selection itself is a causal semantic witness rather than a fixed label.
sed -e 's|gooo://proof/source-evidence|gooo://proof/source-evidence-v2|' \
  -e 's|kind=EVIDENCE_DIGEST_MISMATCH;rank=4|kind=EVIDENCE_DIGEST_MISMATCH;rank=7|' \
  -e 's|kind=SOURCE_RECONSTRUCTION_MISMATCH;rank=7|kind=SOURCE_RECONSTRUCTION_MISMATCH;rank=4|' \
  "$source_repo" > "$output/semantic-intervention.gooo"
{
...
for path in "${paths[@]}"; do
  ...
  case "$path" in
    ...
    policy-*.gooo) role="policy-fixture-source";;
...
policy_fixture_total=4
policy_fixture_observed=0
assert_policy_fixture_rejected() {
	local name="$1" source="$2" expected="$3" observed_issue="${4:-}" log="$output/$1-policy-check.log"
	if go run ./cmd/language-proof-carrying-artifact-verifier -policy-fixture-bundle "$output/bundle.json" -policy-artifact-path artifact.json -policy-source-path "$source" -policy-operation-path operation-receipt.json -policy-recipe-path recipe.json -policy-head "$HEAD_SHA" -policy-observed-issue "$observed_issue" -policy-output "$output/$1-policy-report.json" > "$log" 2>&1; then
		echo "$name policy fixture unexpectedly passed verifyArtifact" >&2
		exit 1
	fi
	rg -F "$expected" "$log"
	jq -e '.decision == "FAIL_CLOSED" and (.receipt_digest | test("^sha256:[0-9a-f]{64}$")) and (.artifact_digest | test("^sha256:[0-9a-f]{64}$")) and (.policy_claim_status != "") and (.policy_claim_coordinate.stage == "CONSUME_POLICY" or .policy_claim_coordinate.stage == "CONSUME_CLAIMS")' "$output/$name-policy-report.json"
	policy_fixture_observed=$((policy_fixture_observed + 1))
}
assert_policy_fixture_rejected "policy-missing" policy-missing.gooo 'stage=CONSUME_POLICY step=parse-policy reason=CASE_ENVELOPE_POLICY_MISSING'
assert_policy_fixture_rejected "policy-malformed-rank" policy-malformed-rank.gooo 'stage=CONSUME_POLICY step=parse-policy reason=CASE_ENVELOPE_POLICY_MALFORMED'
assert_policy_fixture_rejected "policy-duplicate-rank" policy-duplicate-rank.gooo 'stage=CONSUME_POLICY step=validate-policy reason=CASE_ENVELOPE_POLICY_DUPLICATE_RANK'
assert_policy_fixture_rejected "policy-unknown-issue" policy-unknown-issue.gooo 'stage=CONSUME_POLICY step=select-issue reason=CASE_ENVELOPE_POLICY_UNKNOWN_ISSUE' 'UNKNOWN_OBSERVED_ISSUE'
test "$policy_fixture_observed" -eq "$policy_fixture_total"
jq -n --argjson total "$policy_fixture_total" --argjson observed "$policy_fixture_observed" '{schema:"gooo/case-envelope-policy-fixtures/v2",version:2,total:$total,observed:$observed,rows:11,ranks:11,verify_artifact_path:true,bundle_reconstructed:true,consumer_selection_source:"raw-gooo",fixtures:["policy-missing","policy-malformed-rank","policy-duplicate-rank","policy-unknown-issue"]}' > "$output/policy-fixture-ledger.json"
go run ./cmd/language-proof-carrying-artifact-verifier -claim-state-expectations "$output/claim-state-expectations.json"
jq -e '.schema == "gooo/language-proof-carrying-artifact-claim-state-expectations/v2" and .version == 2 and .fixed_denominator == 96 and (.phases.PRELIMINARY.fixed_denominator == 96) and (.phases.FINAL.fixed_denominator == 96) and (.phases.PRELIMINARY.cases | length) == 16 and (.phases.FINAL.cases | length) == 16 and ([.phases.PRELIMINARY.cases[].states[]] | length) == 96 and ([.phases.FINAL.cases[].states[]] | length) == 96 and .phases.PRELIMINARY.totals == {discharged:53,open:28,refuted:15} and .phases.FINAL.totals == {discharged:53,open:27,refuted:16} and .phase_transitions == [{from_phase:"PRELIMINARY",to_phase:"FINAL",case_id:"bytes-only-no-authority",claim_id:"recipe-match",from:"OPEN",to:"REFUTED",coordinate:{stage:"CONSUME_INPUT",step:"external-evidence",reason:"ARTIFACT_BYTES_NOT_AUTHORITY"},basis:"FINAL_RECHECK_OBSERVES_MISSING_EXTERNAL_EVIDENCE"}]' "$output/claim-state-expectations.json"
jq -e '([.claim_adjudications[] | select(.phase == "PRELIMINARY")] | length) == 5 and ([.claim_adjudications[] | select(.phase == "FINAL")] | length) == 5 and ([.claim_adjudications[] | select(.case_id == "valid-proof-carrying-artifact" and .claim_id == "case-envelope-policy-bound" and .coordinate == {stage:"GENERATE",step:"policy-evidence",reason:"CASE_ENVELOPE_POLICY_SOURCE_CARRIED"} and .evidence_digest_count == 1)] | length) == 2 and ([.claim_adjudications[] | select(.case_id == "missing-operation-evidence" and .claim_id == "operation-receipt-bound" and .coordinate == {stage:"CONSUME_EVIDENCE",step:"operation-evidence",reason:"PROOF_EVIDENCE_MISSING"} and .evidence_digest_count == 1)] | length) == 2 and ([.claim_adjudications[] | select(.case_id == "missing-operation-evidence" and .claim_id == "recipe-match" and .coordinate == {stage:"CONSUME_RECIPE",step:"recipe-evidence",reason:"RECIPE_OPERATION_EVIDENCE_MISSING"} and .evidence_digest_count == 3)] | length) == 2 and ([.claim_adjudications[] | select(.case_id == "unrelated-evidence-tamper" and .claim_id == "no-byte-authority" and .coordinate == {stage:"CONSUME_INVARIANT",step:"invariant-evidence",reason:"INVARIANT_EVIDENCE_NOT_PRESERVED"} and .evidence_digest_count == 1)] | length) == 2 and ([.claim_adjudications[] | select(.case_id == "unrelated-evidence-tamper" and .claim_id == "recipe-match" and .coordinate == {stage:"CONSUME_RECIPE",step:"recipe-evidence",reason:"RECIPE_INVARIANT_EVIDENCE_NOT_RESOLVED"} and .evidence_digest_count == 3)] | length) == 2' "$output/claim-state-expectations.json"
...
jq -e '.conformance_decision == "FAIL_CLOSED" and .conformance_resolution == "LOWER_RESOLUTION" and .conformance_reason == "CONSUMER_RECHECK_NOT_OBSERVED" and .conformance_coordinate.stage == "CONSUME_BUNDLE" and .conformance_coordinate.step == "consumer-recheck" and .artifact_use_authority == "" and .summary.claim_templates == 6 and .summary.claim_instances == 96 and .summary.case_discharged_claims == 53 and .summary.case_open_claims == 28 and .summary.case_refuted_claims == 15 and .summary.bundle_only_verification == 1 and .summary.consumer_rechecks == 0 and .proof_summary == {phase:"PRELIMINARY",proofs:3,evidence_validated:3,evidence_validated_total:3,observed_state:1,observed_state_total:3,open:2,open_total:3,discharged:0,discharged_total:3,authority:0,authority_total:1} and ([.indicators[] | select(.satisfied == false)] | length) == 1 and ([.cases[] | select(.id == "valid-proof-carrying-artifact") | .policy | select(.raw_source_digest != "" and .semantic_digest != "" and .unique_issue_rows == 11 and .unique_rank_rows == 11 and (.issue_rows | length) == 11 and .selection_operation == "SELECT_LOWEST_DECLARED_RANK" and .selected_issue == "NONE" and .selected_rank == 0 and (.observed_issue_set | length) == 0 and .observed_issue_count == 0 and (.observed_issue_membership_digest | test("^sha256:[0-9a-f]{64}$")))] | length) == 1' "$output/bundle-preliminary-report.json"
...
jq -e --slurpfile final "$output/bundle-report.json" '([range(0;16) as $i | range(0;6) as $j | select(.cases[$i].claims[$j].status != $final[0].cases[$i].claims[$j].status) | {case_id:.cases[$i].id,claim_id:.cases[$i].claims[$j].id,from:.cases[$i].claims[$j].status,to:$final[0].cases[$i].claims[$j].status,stage:$final[0].cases[$i].coordinate.stage,step:$final[0].cases[$i].coordinate.step,reason:$final[0].cases[$i].coordinate.reason}] == [{case_id:"bytes-only-no-authority",claim_id:"recipe-match",from:"OPEN",to:"REFUTED",stage:"CONSUME_INPUT",step:"external-evidence",reason:"ARTIFACT_BYTES_NOT_AUTHORITY"}]) and ($final[0].summary.case_discharged_claims == 53 and $final[0].summary.case_open_claims == 27 and $final[0].summary.case_refuted_claims == 16)' "$output/bundle-preliminary-report.json"
...
assert_preliminary_rejected "preliminary-summary-bundle-mode" '.summary.bundle_only_verification = 0' 'stage=EVALUATE step=preliminary-summary reason=PRELIMINARY_SUMMARY_MISMATCH'
assert_preliminary_rejected "preliminary-summary-consumer-mode" '.summary.consumer_rechecks = 1' 'stage=EVALUATE step=preliminary-summary reason=PRELIMINARY_SUMMARY_MISMATCH'
go run ./cmd/language-proof-carrying-artifact-verifier -claim-state-tamper "$output/bundle-preliminary-report.json" -claim-state-case valid-proof-carrying-artifact -claim-state-id source-bytes-bound -claim-state OPEN -output "$output/preliminary-claim-state-tamper.json"
if go run ./cmd/language-proof-carrying-artifact-verifier -check "$output/preliminary-claim-state-tamper.json" > "$output/preliminary-claim-state-tamper-check.log" 2>&1; then
  echo "preliminary claim-state tamper unexpectedly validated by CLI" >&2
  exit 1
fi
rg -F 'stage=VERIFY_CLAIM_STATES step=case-claim-state reason=CLAIM_STATE_EXPECTATION_MISMATCH' "$output/preliminary-claim-state-tamper-check.log"
rg -F '"case_id":"valid-proof-carrying-artifact"' "$output/preliminary-claim-state-tamper-check.log"
if go run ./cmd/language-proof-carrying-artifact-consumer -bundle "$output/bundle.json" -report "$output/preliminary-claim-state-tamper.json" -target artifact.json -out "$output/preliminary-claim-state-tamper-consumer-receipt.json" > "$output/preliminary-claim-state-tamper-consumer.log" 2>&1; then
  echo "preliminary claim-state tamper unexpectedly lifted by consumer" >&2
  exit 1
fi
rg -F 'ATTESTATION_MISMATCH' "$output/preliminary-claim-state-tamper-consumer.log"
rg -F 'stage=VERIFY_CLAIM_STATES step=case-claim-state reason=CLAIM_STATE_EXPECTATION_MISMATCH' "$output/preliminary-claim-state-tamper-consumer.log"
preliminary_negative_fixture_observed=$((preliminary_negative_fixture_observed + 1))
go run ./cmd/language-proof-carrying-artifact-verifier -claim-adjudication-tamper "$output/bundle-preliminary-report.json" -claim-adjudication-case missing-operation-evidence -claim-adjudication-id operation-receipt-bound -claim-adjudication-other-case unrelated-evidence-tamper -claim-adjudication-other-id no-byte-authority -output "$output/preliminary-claim-adjudication-swap.json"
if go run ./cmd/language-proof-carrying-artifact-verifier -check "$output/preliminary-claim-adjudication-swap.json" > "$output/preliminary-claim-adjudication-swap-check.log" 2>&1; then
  echo "preliminary claim-adjudication swap unexpectedly validated by CLI" >&2
  exit 1
fi
rg -F 'stage=VERIFY_CLAIM_STATES step=claim-adjudication reason=CLAIM_ADJUDICATION_EXPECTATION_MISMATCH' "$output/preliminary-claim-adjudication-swap-check.log"
if go run ./cmd/language-proof-carrying-artifact-consumer -bundle "$output/bundle.json" -report "$output/preliminary-claim-adjudication-swap.json" -target artifact.json -out "$output/preliminary-claim-adjudication-swap-consumer-receipt.json" > "$output/preliminary-claim-adjudication-swap-consumer.log" 2>&1; then
  echo "preliminary claim-adjudication swap unexpectedly lifted by consumer" >&2
  exit 1
fi
rg -F 'ATTESTATION_MISMATCH' "$output/preliminary-claim-adjudication-swap-consumer.log"
...
go run ./cmd/language-proof-carrying-artifact-verifier -claim-state-tamper "$output/bundle-report.json" -claim-state-case valid-proof-carrying-artifact -claim-state-id source-bytes-bound -claim-state OPEN -output "$output/final-claim-state-tamper.json"
if go run ./cmd/language-proof-carrying-artifact-verifier -check "$output/final-claim-state-tamper.json" > "$output/final-claim-state-tamper-check.log" 2>&1; then
  echo "final claim-state tamper unexpectedly validated by CLI" >&2
  exit 1
fi
rg -F 'stage=VERIFY_CLAIM_STATES step=case-claim-state reason=CLAIM_STATE_EXPECTATION_MISMATCH' "$output/final-claim-state-tamper-check.log"
rg -F '"phase":"FINAL"' "$output/final-claim-state-tamper-check.log"
if go run ./cmd/language-proof-carrying-artifact-consumer -bundle "$output/bundle.json" -report "$output/final-claim-state-tamper.json" -target artifact.json -out "$output/final-claim-state-tamper-consumer-receipt.json" > "$output/final-claim-state-tamper-consumer.log" 2>&1; then
  echo "final claim-state tamper unexpectedly consumed" >&2
  exit 1
fi
rg -F 'ATTESTATION_MISMATCH' "$output/final-claim-state-tamper-consumer.log"
rg -F 'stage=VERIFY_CLAIM_STATES step=case-claim-state reason=CLAIM_STATE_EXPECTATION_MISMATCH' "$output/final-claim-state-tamper-consumer.log"
go run ./cmd/language-proof-carrying-artifact-verifier -claim-adjudication-tamper "$output/bundle-report.json" -claim-adjudication-case missing-operation-evidence -claim-adjudication-id operation-receipt-bound -claim-adjudication-other-case unrelated-evidence-tamper -claim-adjudication-other-id no-byte-authority -output "$output/final-claim-adjudication-swap.json"
if go run ./cmd/language-proof-carrying-artifact-verifier -check "$output/final-claim-adjudication-swap.json" > "$output/final-claim-adjudication-swap-check.log" 2>&1; then
  echo "final claim-adjudication swap unexpectedly validated by CLI" >&2
  exit 1
fi
rg -F 'stage=VERIFY_CLAIM_STATES step=claim-adjudication reason=CLAIM_ADJUDICATION_EXPECTATION_MISMATCH' "$output/final-claim-adjudication-swap-check.log"
if go run ./cmd/language-proof-carrying-artifact-consumer -bundle "$output/bundle.json" -report "$output/final-claim-adjudication-swap.json" -target artifact.json -out "$output/final-claim-adjudication-swap-consumer-receipt.json" > "$output/final-claim-adjudication-swap-consumer.log" 2>&1; then
  echo "final claim-adjudication swap unexpectedly consumed" >&2
  exit 1
fi
rg -F 'ATTESTATION_MISMATCH' "$output/final-claim-adjudication-swap-consumer.log"
rg -F 'stage=VERIFY_CLAIM_STATES step=claim-adjudication reason=CLAIM_ADJUDICATION_EXPECTATION_MISMATCH' "$output/final-claim-adjudication-swap-consumer.log"
claim_adjudication_fixture_observed=$((claim_adjudication_fixture_observed + 1))
test "$claim_adjudication_fixture_observed" -eq "$claim_adjudication_fixture_total"
...
jq -e --slurpfile receipt "$output/consumer-receipt.json" '.consumer_receipt == $receipt[0] and .conformance_decision == "PASS" and .conformance_resolution == "EXACT" and .checkout_binding_scope == "BUNDLE_HISTORICAL_SUBJECT_BINDING" and .summary.bundle_only_verification == 1 and .summary.consumer_rechecks == 1 and .summary.claim_templates == 6 and .summary.claim_instances == 96 and .summary.case_discharged_claims == 53 and .summary.case_open_claims == 27 and .summary.case_refuted_claims == 16 and .summary.net_repository_state_unchanged == 1 and .summary.unknown_authority_observations == 1 and (.consumer_receipt.policy_raw_source_digest != "") and (.consumer_receipt.policy_semantic_digest != "") and .consumer_receipt.policy_unique_issue_rows == 11 and .consumer_receipt.policy_unique_rank_rows == 11 and .consumer_receipt.policy_row_total == 11 and .consumer_receipt.policy_selection_operation == "SELECT_LOWEST_DECLARED_RANK" and (.consumer_receipt.policy_observed_issue_set | length) == 0 and .consumer_receipt.policy_observed_issue_count == 0 and (.consumer_receipt.policy_membership_digest | test("^sha256:[0-9a-f]{64}$")) and (.consumer_receipt.policy_claim_transition_digest | test("^sha256:[0-9a-f]{64}$")) and .consumer_receipt.policy_selected_issue == "NONE" and .consumer_receipt.policy_selected_rank == 0 and (.consumer_receipt.case_envelope_digest | test("^sha256:[0-9a-f]{64}$")) and ([.interventions[] | select(.kind == "SEMANTIC" and .semantic_digest_changed == true and .operation_receipt_changed == true and .evidence_links_changed == true and .claim_transitions_changed == true and .policy_membership_preserved == true and .policy_semantic_digest_before != .policy_semantic_digest_after and .policy_selection_changed == true and .policy_selection_before == "EVIDENCE_DIGEST_MISMATCH" and .policy_selection_after == "SOURCE_RECONSTRUCTION_MISMATCH")] | length) == 1 and ([.interventions[] | select(.kind == "NONSEMANTIC" and .raw_digest_changed == true and .semantic_digest_preserved == true and .consumer_decision_preserved == true and .policy_membership_preserved == true and .policy_semantic_digest_before == .policy_semantic_digest_after and .policy_selection_preserved == true and .policy_selection_before == "EVIDENCE_DIGEST_MISMATCH" and .policy_selection_after == "EVIDENCE_DIGEST_MISMATCH")] | length) == 1' "$output/bundle-report.json"
jq -e '([.cases[] | select(.id == "valid-proof-carrying-artifact") | .policy][0]) as $policy | ($policy.raw_source_digest == .consumer_receipt.policy_raw_source_digest and $policy.semantic_digest == .consumer_receipt.policy_semantic_digest and $policy.unique_issue_rows == 11 and $policy.unique_rank_rows == 11 and ($policy.issue_rows | length) == 11 and ([ $policy.issue_rows[].kind ] | unique | length) == 11 and ([ $policy.issue_rows[].rank ] | unique | length) == 11 and $policy.selected_issue == "NONE" and $policy.selected_rank == 0 and .consumer_receipt.case_envelope_digest == ([.cases[] | select(.id == "valid-proof-carrying-artifact") | .envelope_digest][0]))' "$output/bundle-report.json"
jq -e '.summary.tampered_rejections == 1 and .summary.coherent_tamper_rejections == 1 and .summary.missing_evidence_rejections == 1 and .summary.byte_only_denials == 1 and .summary.recipe_rejections == 1 and .summary.recipe_only_rejections == 1 and .summary.missing_attachment_rejections == 1 and .summary.wrong_attachment_rejections == 1 and .summary.unrelated_evidence_rejections == 1 and .summary.stale_head_rejections == 1 and .summary.unauthorized_consumer_denials == 1 and .summary.coherent_claim_structure_rejections == 4 and .summary.final_ledger_open_claims == 6 and .summary.final_ledger_discharged_claims == 6 and .summary.accepted_transitions == 6 and ([.indicators[] | select(.satisfied == false)] | length) == 0 and ([.claim_transitions[] | select(.from == "CARRIED" and .to == "PRESERVED")] | length) == 5 and ([.claim_transitions[] | select(.claim_id == "consumer-authority" and .capability == "ARTIFACT_USE" and .from == "NONE" and .to == "READ_ONLY_CONSUMPTION")] | length) == 1 and ([.cases[] | select(.id == "coherent-tamper-reconstruction" and .observed_reason == "OPERATION_RECONSTRUCTION_MISMATCH")] | length) == 1 and ([.cases[] | select(.id == "recipe-only-mismatch" and .observed_reason == "INDEPENDENT_RECIPE_MISMATCH" and .coordinate.stage == "CONSUME_RECIPE" and .coordinate.step == "recipe")] | length) == 1 and ([.cases[] | select(.id == "unauthorized-consumer" and .observed_reason == "UNAUTHORIZED_CONSUMER_NOT_ATTESTED" and .consumer_output_exists == false and .consumer_output_digest == "" and .consumer_error_class == "ATTESTATION_MISMATCH")] | length) == 1' "$output/bundle-report.json"
...
jq -e '.artifact_use_authority == "READ_ONLY_CONSUMPTION" and .summary.bundle_only_verification == 1 and .summary.consumer_rechecks == 1' "$output/bundle-report.json"
jq -e 'def claim($case;$id): [.cases[] | select(.id == $case) | .claims[] | select(.id == $id)]; (claim("missing-operation-evidence";"operation-receipt-bound") | map(select(.status == "REFUTED" and .resolution == "INVARIANT_ONLY" and .reason == "CLAIM_REFUTED" and .coordinate == {stage:"CONSUME_EVIDENCE",step:"operation-evidence",reason:"PROOF_EVIDENCE_MISSING"} and (.evidence_digests | length) == 1)) | length) == 1 and (claim("missing-operation-evidence";"recipe-match") | map(select(.status == "REFUTED" and .coordinate == {stage:"CONSUME_RECIPE",step:"recipe-evidence",reason:"RECIPE_OPERATION_EVIDENCE_MISSING"} and (.evidence_digests | length) == 3)) | length) == 1 and (claim("unrelated-evidence-tamper";"no-byte-authority") | map(select(.status == "REFUTED" and .resolution == "INVARIANT_ONLY" and .reason == "CLAIM_REFUTED" and .coordinate == {stage:"CONSUME_INVARIANT",step:"invariant-evidence",reason:"INVARIANT_EVIDENCE_NOT_PRESERVED"} and (.evidence_digests | length) == 1)) | length) == 1 and (claim("unrelated-evidence-tamper";"recipe-match") | map(select(.status == "OPEN" and .resolution == "LOWER_RESOLUTION" and .reason == "CLAIM_PENDING" and .coordinate == {stage:"CONSUME_RECIPE",step:"recipe-evidence",reason:"RECIPE_INVARIANT_EVIDENCE_NOT_RESOLVED"} and (.evidence_digests | length) == 3)) | length) == 1' "$output/bundle-report.json"
...
jq -n --argjson total "$claim_adjudication_fixture_total" --argjson observed "$claim_adjudication_fixture_observed" '{schema:"gooo/claim-adjudication-causal-fixtures/v1",total:$total,observed:$observed,preliminary_and_final_checked:true,coherent_swap_rejections:2}' > "$output/claim-adjudication-fixture-ledger.json"
