...
jq -e '.proof_summary == "observed evidence 3/3; open proofs 0/0; discharged proofs 3/3; authority READ_ONLY" and ([.proofs[] | select(.phase == "FINAL" and .state == "DISCHARGED" and .evidence_validated == true and .passed == true and .consumer_gate_open == false)] | length) == 3' "$output/bundle-report.json"
