...
jq -e 'length == 16 and ([.[].states[] | select(. == "DISCHARGED")] | length) == 42 and ([.[].states[] | select(. == "OPEN")] | length) == 22 and ([.[].states[] | select(. == "REFUTED")] | length) == 16 and ([.[].states[]] | length) == 80' "$output/claim-state-expectations.json"
