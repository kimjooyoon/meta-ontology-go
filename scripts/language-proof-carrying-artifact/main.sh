...
jq -cn '{raw_artifact_bytes_without_attestation:true,authority:"READ_ONLY_CONSUMPTION"}' > "$output/unauthorized-consumer.json"
...
go run ./cmd/language-proof-carrying-artifact-verifier -bundle "$output/bundle.json" -output "$output/bundle-report.json"

jq -e --slurpfile bundle_report "$output/bundle-report.json" '(.conformance_decision == $bundle_report[0].conformance_decision) and (.conformance_resolution == $bundle_report[0].conformance_resolution) and (.subject_artifact_decision == $bundle_report[0].subject_artifact_decision) and (.summary.cases_satisfied == $bundle_report[0].summary.cases_satisfied)' "$output/report.json"
...
go run ./cmd/language-proof-carrying-artifact-consumer -bundle "$output/bundle.json" -report "$output/bundle-report.json" -target artifact.json -out "$output/consumer-receipt.json"
jq -e --slurpfile receipt "$output/consumer-receipt.json" '.consumer_receipt == $receipt[0] and .summary.bundle_only_verification == 1 and .summary.consumer_rechecks == 1 and .summary.claim_templates == 5 and .summary.claim_instances == 60 and .summary.accepted_transitions == 5 and .summary.case_discharged_claims == 34 and .summary.case_open_claims == 13 and .summary.case_refuted_claims == 13 and .summary.final_ledger_open_claims == 5 and .summary.final_ledger_discharged_claims == 5 and .summary.recipe_rejections == 1 and .summary.recipe_only_rejections == 1 and .summary.missing_attachment_rejections == 1 and .summary.wrong_attachment_rejections == 1 and .summary.unrelated_evidence_rejections == 1 and .summary.stale_head_rejections == 1 and .summary.unauthorized_consumer_denials == 1 and .summary.net_repository_state_unchanged == 1 and .write_set.observed_scope == "NET_BEFORE_AFTER_TRACKED_AND_UNTRACKED" and .write_set.transient_writes_unknown == true and .authority_observation == "UNKNOWN_GLOBAL_TRANSIENT_SCOPE" and ([.indicators[] | select(.satisfied == false)] | length) == 0 and ([.claim_transitions[] | select(.from == "CARRIED" and .to == "PRESERVED")] | length) == 4 and ([.claim_transitions[] | select(.claim_id == "consumer-authority" and .capability == "ARTIFACT_USE" and .from == "NONE" and .to == "READ_ONLY_CONSUMPTION")] | length) == 1 and ([.interventions[] | select(.kind == "SEMANTIC" and .semantic_digest_changed == true and .operation_receipt_changed == true and .evidence_links_changed == true and .claim_transitions_changed == true)] | length) == 1 and ([.interventions[] | select(.kind == "NONSEMANTIC" and .raw_digest_changed == true and .semantic_digest_preserved == true and .consumer_decision_preserved == true)] | length) == 1 and ([.cases[] | select(.id == "coherent-tamper-reconstruction" and .observed_reason == "OPERATION_RECONSTRUCTION_MISMATCH")] | length) == 1 and ([.cases[] | select(.id == "recipe-only-mismatch" and .observed_reason == "RECIPE_CLAIM_ONLY_MISMATCH")] | length) == 1 and ([.cases[] | select(.id == "unauthorized-consumer" and .observed_reason == "UNAUTHORIZED_CONSUMER_NOT_ATTESTED")] | length) == 1' "$output/bundle-report.json"
