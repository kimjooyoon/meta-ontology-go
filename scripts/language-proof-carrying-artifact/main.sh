...
sed 's|kind=RECIPE_MISMATCH;rank=11|kind=UNKNOWN_ISSUE;rank=11|' "$output/source.gooo" > "$output/policy-unknown-issue.gooo"
assert_policy_rejected() {
  local name="$1" source="$2" expected="$3" observed_issue="${4:-}" log="$output/$1-policy-check.log"
  if go run ./cmd/language-proof-carrying-artifact-verifier -policy-check "$source" -policy-observed-issue "$observed_issue" -policy-output "$output/$1-policy-report.json" > "$log" 2>&1; then
    echo "$name policy unexpectedly validated" >&2
    exit 1
  fi
  rg -F "$expected" "$log"
  policy_fixture_observed=$((policy_fixture_observed + 1))
}
assert_policy_rejected "policy-missing" "$output/policy-missing.gooo" 'stage=CONSUME_POLICY step=parse-policy reason=CASE_ENVELOPE_POLICY_MISSING'
assert_policy_rejected "policy-malformed-rank" "$output/policy-malformed-rank.gooo" 'stage=CONSUME_POLICY step=parse-policy reason=CASE_ENVELOPE_POLICY_MALFORMED'
assert_policy_rejected "policy-duplicate-rank" "$output/policy-duplicate-rank.gooo" 'stage=CONSUME_POLICY step=validate-policy reason=CASE_ENVELOPE_POLICY_DUPLICATE_RANK'
assert_policy_rejected "policy-unknown-issue" "$output/policy-unknown-issue.gooo" 'stage=CONSUME_POLICY step=select-issue reason=CASE_ENVELOPE_POLICY_UNKNOWN_ISSUE' 'RECIPE_MISMATCH'
test "$policy_fixture_observed" -eq "$policy_fixture_total"
jq -n --argjson total "$policy_fixture_total" --argjson observed "$policy_fixture_observed" '{schema:"gooo/case-envelope-policy-fixtures/v1",total:$total,observed:$observed,rows:11,ranks:11,consumer_selection_source:"raw-gooo"}' > "$output/policy-fixture-ledger.json"
go run ./cmd/language-proof-carrying-artifact-verifier -claim-state-expectations "$output/claim-state-expectations.json"
jq -e '.schema == "gooo/language-proof-carrying-artifact-claim-state-expectations/v1" and .fixed_denominator == 80 and (.phases.PRELIMINARY.fixed_denominator == 80) and (.phases.FINAL.fixed_denominator == 80) and (.phases.PRELIMINARY.cases | length) == 16 and (.phases.FINAL.cases | length) == 16 and ([.phases.PRELIMINARY.cases[].states[]] | length) == 80 and ([.phases.FINAL.cases[].states[]] | length) == 80 and .phases.PRELIMINARY.totals == {discharged:42,open:23,refuted:15} and .phases.FINAL.totals == {discharged:42,open:22,refuted:16} and .phase_transitions == [{from_phase:"PRELIMINARY",to_phase:"FINAL",case_id:"bytes-only-no-authority",claim_id:"recipe-match",from:"OPEN",to:"REFUTED",coordinate:{stage:"CONSUME_INPUT",step:"external-evidence",reason:"ARTIFACT_BYTES_NOT_AUTHORITY"},basis:"FINAL_RECHECK_OBSERVES_MISSING_EXTERNAL_EVIDENCE"}]' "$output/claim-state-expectations.json"
jq -e '([.claim_adjudications[] | select(.phase == "PRELIMINARY")] | length) == 4 and ([.claim_adjudications[] | select(.phase == "FINAL")] | length) == 4 and ([.claim_adjudications[] | select(.case_id == "missing-operation-evidence" and .claim_id == "operation-receipt-bound" and .coordinate == {stage:"CONSUME_EVIDENCE",step:"operation-evidence",reason:"PROOF_EVIDENCE_MISSING"} and .evidence_digest_count == 1)] | length) == 2 and ([.claim_adjudications[] | select(.case_id == "missing-operation-evidence" and .claim_id == "recipe-match" and .coordinate == {stage:"CONSUME_RECIPE",step:"recipe-evidence",reason:"RECIPE_OPERATION_EVIDENCE_MISSING"} and .evidence_digest_count == 3)] | length) == 2 and ([.claim_adjudications[] | select(.case_id == "unrelated-evidence-tamper" and .claim_id == "no-byte-authority" and .coordinate == {stage:"CONSUME_INVARIANT",step:"invariant-evidence",reason:"INVARIANT_EVIDENCE_NOT_PRESERVED"} and .evidence_digest_count == 1)] | length) == 2 and ([.claim_adjudications[] | select(.case_id == "unrelated-evidence-tamper" and .claim_id == "recipe-match" and .coordinate == {stage:"CONSUME_RECIPE",step:"recipe-evidence",reason:"RECIPE_INVARIANT_EVIDENCE_NOT_RESOLVED"} and .evidence_digest_count == 3)] | length) == 2' "$output/claim-state-expectations.json"
...
jq -e --slurpfile receipt "$output/consumer-receipt.json" '.consumer_receipt == $receipt[0] and .conformance_decision == "PASS" and .conformance_resolution == "EXACT" and .checkout_binding_scope == "BUNDLE_HISTORICAL_SUBJECT_BINDING" and .summary.bundle_only_verification == 1 and .summary.consumer_rechecks == 1 and .summary.claim_templates == 5 and .summary.claim_instances == 80 and .summary.case_discharged_claims == 42 and .summary.case_open_claims == 22 and .summary.case_refuted_claims == 16 and .summary.net_repository_state_unchanged == 1 and .summary.unknown_authority_observations == 1 and (.consumer_receipt.policy_raw_source_digest != "") and (.consumer_receipt.policy_semantic_digest != "") and .consumer_receipt.policy_unique_issue_rows == 11 and .consumer_receipt.policy_unique_rank_rows == 11 and .consumer_receipt.policy_row_total == 11 and .consumer_receipt.policy_selection_operation == "SELECT_LOWEST_DECLARED_RANK" and (.consumer_receipt.policy_observed_issue_set | length) == 0 and .consumer_receipt.policy_selected_issue == "NONE" and .consumer_receipt.policy_selected_rank == 0 and (.consumer_receipt.case_envelope_digest | test("^sha256:[0-9a-f]{64}$")) and ([.interventions[] | select(.kind == "SEMANTIC" and .semantic_digest_changed == true and .operation_receipt_changed == true and .evidence_links_changed == true and .claim_transitions_changed == true and .policy_semantic_digest_before != .policy_semantic_digest_after and .policy_selection_changed == true and .policy_selection_before == "EVIDENCE_DIGEST_MISMATCH" and .policy_selection_after == "SOURCE_RECONSTRUCTION_MISMATCH")] | length) == 1 and ([.interventions[] | select(.kind == "NONSEMANTIC" and .raw_digest_changed == true and .semantic_digest_preserved == true and .consumer_decision_preserved == true and .policy_semantic_digest_before == .policy_semantic_digest_after and .policy_selection_preserved == true and .policy_selection_before == "EVIDENCE_DIGEST_MISMATCH" and .policy_selection_after == "EVIDENCE_DIGEST_MISMATCH")] | length) == 1' "$output/bundle-report.json"
