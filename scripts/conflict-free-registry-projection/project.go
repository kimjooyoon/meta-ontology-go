...
func renderOutputs(root, outputDir string, loaded []LoadedManifest) (map[string][]byte, DigestFile, error) {
	...
	catalogRaw, err := renderJSON(catalogFile{Schema: "gooo/manual-source-registration-edit-free-registry-catalog/v1", SectionDigest: sectionDigest(projection.Catalog), Entries: projection.Catalog})
	if err != nil {
		return nil, DigestFile{}, err
	}
	corpusRaw, err := renderJSON(resourceFile{Schema: "gooo/manual-source-registration-edit-free-registry-corpus/v1", SectionDigest: sectionDigest(projection.Corpus), Entries: projection.Corpus})
	if err != nil {
		return nil, DigestFile{}, err
	}
	registryRaw, err := renderJSON(resourceFile{Schema: "gooo/manual-source-registration-edit-free-registry-registry/v1", SectionDigest: sectionDigest(projection.Registry), Entries: projection.Registry})
	if err != nil {
		return nil, DigestFile{}, err
	}
	denominatorRaw, err := renderJSON(denominatorFile{Schema: "gooo/manual-source-registration-edit-free-registry-denominator/v1", SectionDigest: sectionDigest(projection.Denominator), Entries: projection.Denominator})
	if err != nil {
		return nil, DigestFile{}, err
	}
	readmeRaw := renderREADME(projection, loaded)
	manifestDigests := manifestDigestFile{Schema: "gooo/manual-source-registration-edit-free-registry-manifests/v1", Manifests: make([]ManifestDigest, 0, len(loaded))}
...
func renderREADME(projection Projection, loaded []LoadedManifest) []byte {
	...
	builder.WriteString("# Manual-source-registration-edit-free registry projection\n\n")
