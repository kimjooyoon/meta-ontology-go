...
jq -e '
  ...
  .summary.coordinates_satisfied == 12 and .summary.coordinates_total == 12 and
  .summary.fixed_denominator == 12 and .summary.zero_ambiguity_cases == 1 and
...
jq -e '
  ...
  ([.claims[] | select(.case_id == "over-budget-ambiguity" and .to == "LOWER_RESOLUTION")] | length) == 1 and
  ([.claims[] | select(.case_id == "unknown-ambiguity" and .to == "LOWER_RESOLUTION")] | length) == 1
' "$output/receipt-first.json"
jq -e '.decision == "PASS" and .resolution == "EXACT" and .fixed_denominator == 12 and .cases_satisfied == 4 and .coordinates_satisfied == 12 and .repository_writes == 0 and .mutation_authority == false' "$output/judge-first.json"
