...
evolution_observed=$(jq -c '{schema,authority,old_artifact_digest,new_artifact_digest,denominator_id,denominator_unchanged,fixed_claim_total_before,fixed_claim_total_after,change_kind,claim_recreated_due_only_to_raw_digest,claim_recreated_due_only_to_raw_digest_total,new_expectation_producer_exact:([.cases[] | select(.new_expectation_producer_exact == true)] | length),new_expectation_consumer_exact:([.cases[] | select(.new_expectation_consumer_exact == true)] | length),cases:(.cases | map({case_id,old_expected_count:(.old_expected_ids|length),producer_old_observed_count:(.producer_old_observed_ids|length),consumer_old_observed_count:(.consumer_old_observed_ids|length),old_artifact_exact,old_producer_consumer_exact,new_expected_count:(.new_expected_ids|length),producer_observed_count:(.producer_observed_ids|length),consumer_observed_count:(.consumer_observed_ids|length),new_expectation_producer_exact,new_expectation_consumer_exact,producer_consumer_exact,claim_recreated_due_only_to_raw_digest,claim_recreated_due_only_to_raw_digest_total,removed_id_count:(.removed_ids|length),added_id_count:(.added_ids|length),proposition_target_change_count:(.proposition_target_changes|length),consumer_proposition_target_change_count:(.consumer_proposition_target_changes|length)}))}' "$evolution")
run_gate "expectation-evolution" "claim-identity" "validate-expectation-evolution" \
  'independent old/new artifact bytes and source rows, denominator unchanged, decision=FIXED_POINT/resolution=EXACT' "$evolution_observed" "EXPECTATION_EVOLUTION_RECEIPT_INVALID" \
  '$evolution[0].schema == "gooo/semantic-delta-claim-expectation-evolution/v2" and $evolution[0].authority == "SOURCE_DERIVED_SEMANTIC_CLAIM_CONTRACT" and ($evolution[0].old_artifact_digest | test("^sha256:[0-9a-f]{64}$")) and ($evolution[0].new_artifact_digest | test("^sha256:[0-9a-f]{64}$")) and $evolution[0].old_artifact_digest != $evolution[0].new_artifact_digest and $evolution[0].new_artifact_digest == $expectation_digest and $evolution[0].old_artifact_bytes > 0 and $evolution[0].new_artifact_bytes > 0 and $evolution[0].denominator_id == "gooo://semantic-delta-receipt-denominator/v2" and $evolution[0].denominator_unchanged == true and $evolution[0].fixed_claim_total_before == 31 and $evolution[0].fixed_claim_total_after == 31 and $evolution[0].change_kind == "STABLE_IDENTITY_V3_EVIDENCE_SEPARATED" and $evolution[0].decision == "FIXED_POINT" and $evolution[0].resolution == "EXACT" and $evolution[0].evolution_rows_independently_reconstructed == 5 and $evolution[0].evolution_rows_total == 5 and $evolution[0].claim_recreated_due_only_to_raw_digest == 0 and ([$evolution[0].cases[].case_id] | sort) == ["ambiguous-match","equivalent","indeterminate","semantic-change","value-program-change"] and ([$evolution[0].cases[] | select(.old_artifact_exact == true and .old_producer_consumer_exact == true and .producer_consumer_exact == true and ((.new_expected_ids | sort) == (.producer_observed_ids | sort)) and ((.new_expected_ids | sort) == (.consumer_observed_ids | sort)))] | length) == 5 and $evolution[0].raw_evidence_changed_on_nonsemantic == 5 and $evolution[0].raw_evidence_nonsemantic_total == 5'

persistent_identity_observed=$(jq -c '{persistent_claim_identity,persistent_claim_identity_total,evolution_claim_rows_independently_reconstructed,evolution_claim_rows_total}' "$evolution")
run_gate "persistent-claim-identity" "claim-identity" "validate-persistent-claim-identity" \
  'stable proposition identity and independent claim-row evolution are 31/31' "$persistent_identity_observed" "PERSISTENT_CLAIM_IDENTITY_MISMATCH" \
  '$evolution[0].persistent_claim_identity == 31 and $evolution[0].persistent_claim_identity_total == 31 and $evolution[0].evolution_claim_rows_independently_reconstructed == 31 and $evolution[0].evolution_claim_rows_total == 31'
...
run_gate "evolution-expectation-binding" "claim-identity" "compare-expectation-with-independent-runtime" \
  ...
  '$evolution[0].new_expectation_producer_exact == 5 and $evolution[0].new_expectation_consumer_exact == 5'
