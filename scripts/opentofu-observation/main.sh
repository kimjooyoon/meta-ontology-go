...
pending_evidence_files=4
output_artifact_files=$((evidence_files_before_reports + pending_evidence_files))
...
reuse_start_ns="$(date +%s%N)"
reuse_rss="$work/reuse-decision.rss"
/usr/bin/time -q -f '%M' -o "$reuse_rss" jq -n --slurpfile baseline "$work/baseline-observation.json" --slurpfile prior "$work/baseline-report.json" \
  ...
  '$baseline[0] as $baseline_observation | $prior[0] as $prior_report | $baseline_observation | .reuse = (.reuse + {request_mode:"REUSE",requests:2,discovered:2,executed:1,reused:1,skipped:0,prior_candidates:1,invalidated:0,decision:"REUSED",reason:"EXACT_PRIOR_RECEIPT",prior_receipt_digest:$prior_report.report_digest,prior_receipt_file_digest:$receipt_file_digest,prior_artifact_manifest_digest:$artifact_manifest_digest,baseline_physical_command_executions:9,baseline_physical_test_executions:2,reuse_physical_command_executions:0,reuse_physical_test_executions:0,prior_receipts_valid:1,reused_artifact_files:$artifact_files,requires_execution:false}) | .cell_evidence_projections.REUSE_ELIGIBILITY=$reuse_cell_projection | .cell_evidence_digests.REUSE_ELIGIBILITY=$reuse_cell_digest | .cell_evidence_projections.HUMAN_REPORT=$reuse_human_projection | .cell_evidence_digests.HUMAN_REPORT=$reuse_human_digest | .prior_receipt={report:$prior_report,receipt_file_digest:$receipt_file_digest,artifact_files:$artifact_files,artifact_digests:$artifact_digests,artifact_manifest_digest:$artifact_manifest_digest}' > "$work/reuse-observation-base.json"
reuse_status=$?
reuse_end_ns="$(date +%s%N)"
test "$reuse_status" -eq 0
reuse_wall_ns=$((reuse_end_ns - reuse_start_ns))
reuse_wall_ms=$(((reuse_wall_ns + 999999) / 1000000))
if ((reuse_wall_ns > 0 && reuse_wall_ms == 0)); then reuse_wall_ms=1; fi
reuse_peak_rss="$(tail -n 1 "$reuse_rss")"
[[ "$reuse_peak_rss" =~ ^[0-9]+$ ]]
jq --argjson wall_ms "$reuse_wall_ms" --argjson peak_rss "$reuse_peak_rss" '.reuse.decision_wall_ms=$wall_ms | .reuse.decision_peak_rss_kib=$peak_rss' "$work/reuse-observation-base.json" > "$out/observation.json"
...
jq -e '.decision=="PASS" and .resolution=="EXACT" and (.cells|length)==12 and .summary.closed_cells==12 and .summary.foundation_closed==4 and .summary.coherence_closed==4 and .summary.regression_closed==4 and .summary.three_user_paths==3 and .summary.executions==2 and .summary.replay_matches==1 and .reuse.request_mode=="REUSE" and .reuse.requests==2 and .reuse.baseline_physical_command_executions==9 and .reuse.baseline_physical_test_executions==2 and .reuse.reuse_physical_command_executions==0 and .reuse.reuse_physical_test_executions==0 and .reuse.prior_candidates==1 and .reuse.prior_receipts_valid==1 and (.reuse.reused_artifact_files|length)==13 and .reuse.reused==1 and .reuse.invalidated==0 and .reuse.skipped==0 and .reuse.requires_execution==false and .repository_writes==0 and .local_test_executions==0 and .promotion_authorized==false' "$out/report.json"
