def expected_paths:
  [
    ...
    "generated-value-projection-report.json"
  ];
...
def digest_binding($input; $path):
  ...
  | {
      ...
      meta_operation: "bind-value-projection-payload-digest",
      path: $path,
      observed: $observed,
      expected: 1,
...
| ([$expected_paths[] as $path | digest_binding($input; $path)] + [
    {
      ...
      meta_operation: "compare-exact-value-manifest-entry-set",
...
| {
    ...
    metric_id: "gooo.metric.guardrail.generated-value-integrity.v1",
    ...
    reason: (if $satisfied == $total
      then "GENERATED_VALUE_PAYLOADS_BOUND"
      else "GENERATED_VALUE_PAYLOAD_BINDING_INCOMPLETE"
      end),
    manifest: {
