...
for file in receipt.json artifact.json schema.json bin/jv; do
  test -f "$producer/$file"
done
chmod +x "$producer/bin/jv"
"$producer/bin/jv" "$producer/schema.json" "$root/examples/symbolic-invocation-schema/accepted.json" > "$work/accepted-validation.txt"
...
jq -n --arg subject "$HEAD_SHA" --arg artifact "$artifact_digest" --arg schema "$schema_digest" --arg tool "$tool_digest" '{
...
jq -e '
  ...
  .summary.coordinates=={satisfied:6,total:6,basis_points:10000} and
  ...
  [.views[]|[.audience,.satisfied,.total]]==[["USER",3,3],["TOOL_AUTHOR",4,4],["GOVERNOR",6,6]] and
...
jq -e '.decision=="FAIL_CLOSED" and .resolution=="LOWER_RESOLUTION" and .reason=="SYMBOLIC_INVOCATION_USECASE_DECISION_UNKNOWN" and .summary.coordinates.satisfied==0 and .summary.coordinates.total==6 and .summary.unknowns==1' "$work/unknown-report.json"
...
jq -e '.decision=="FAIL_CLOSED" and .resolution=="INVARIANT_ONLY" and .reason=="SYMBOLIC_INVOCATION_USECASE_LINK_MISMATCH" and .summary.coordinates.satisfied==0 and .summary.coordinates.total==6' "$work/link-mismatch-report.json"

sha256sum "$work/report.json" "$work/unknown-report.json" "$work/link-mismatch-report.json" > "$work/manifest.sha256"
git diff --exit-code -- "${paths[@]}"
