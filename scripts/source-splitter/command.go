...
func run(args []string) error {
	...
	actualSHA, err := repositorySHA(cfg.root)
	if err != nil {
		return err
	}
	if actualSHA != cfg.sha {
		return fmt.Errorf("repository SHA %s does not match evidence %s", actualSHA, cfg.sha)
	}
	report, err := loadMetricReport(cfg.metrics, cfg.sha)
	if err != nil {
		return err
	}
	indicators := report.splitIndicators()
	if cfg.check {
		return checkSplitPlans(cfg, report, indicators)
	}
	if !containsSubject(indicators, cfg.subject) {
	...
	if err := report.validateTopology(plan); err != nil {
		return err
	}
