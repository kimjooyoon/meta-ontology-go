...
run_debug first SEMANTIC_LOWERED
run_debug second SEMANTIC_LOWERED
...
jq -e '.scope=="DECLARATION_RESOLUTION_ONLY" and .decision=="PASS" and .resolution=="EXACT" and .summary.coordinates.satisfied==13 and .summary.coordinates.total==13 and .summary.replay_matches==1 and .summary.resource_observations==2 and .summary.compiler.go127_runtimes==2 and .summary.unknowns==0 and .summary.refuted_cases==0 and .summary.effects.repository_writes==0 and .summary.effects.mutation_authority==false and .replay.equal==true and (.replay.excluded_fields|length)>0 and (.views[]|select(.audience=="USER")|.satisfied==4 and .total==4) and (.views[]|select(.audience=="TOOL_AUTHOR")|.satisfied==9 and .total==9) and (.views[]|select(.audience=="GOVERNOR")|.satisfied==13 and .total==13)' "$work/report.json"
...
jq -e '.decision=="UNKNOWN" and .resolution=="LOWER_RESOLUTION" and (.unknown_cases|length)==1 and (.unknown_cases[0] | [.stage,.step,.reason,.unknown_class,.next_operation] | all(. != "")) and (.unknown_cases[0].blocked_by|type)=="array" and (.unknown_cases[0].unknown_class=="DIRECT_MISSING" and (.unknown_cases[0].blocked_by|length)==0)' "$work/missing-second-report.json"
jq -e '.decision=="UNKNOWN" and .resolution=="LOWER_RESOLUTION" and (.unknown_cases|length)==1 and (.unknown_cases[0] | [.stage,.step,.reason,.unknown_class,.next_operation] | all(. != "")) and (.unknown_cases[0].blocked_by|type)=="array" and (.unknown_cases[0].unknown_class=="INCOMPLETE_EVIDENCE" and (.unknown_cases[0].blocked_by|length)==0)' "$work/missing-resource-report.json"
jq -e '.decision=="REFUTED" and .resolution=="EXACT" and (.unknown_cases|length)==0 and (.refuted_cases[0].reason=="DEBUG_DETERMINISTIC_DIGEST_CONTRADICTION")' "$work/digest-contradiction-report.json"
...
jq -e '.decision=="REFUTED" and .resolution=="EXACT" and (.refuted_cases[0].reason=="GOOO_GRAPH_ACTIVITY_OR_EDGE_CONTRADICTION") and (.unknown_cases|length)==0' "$work/graph-edge-removal-report.json"
