...
func buildLedgerIndicators(ledger witnessLedger) []ledgerIndicator {
	...
	return []ledgerIndicator{
		...
		pass("coherence.function-meta-coverage", "COHERENCE", "=", itoa(counts.FunctionSourceBindings), itoa(counts.FunctionWitnesses)),
		...
		pass("coherence.root-summary-meta-coverage", "COHERENCE", "=", itoa(counts.RootSummaryIndicators), itoa(rootSummaryCount)),
