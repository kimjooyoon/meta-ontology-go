...
func buildLedger(report sourceReport, expectedSHA string) (witnessLedger, error) {
	...
	observations := unsatisfiedLineDrivers(report.Meta.Indicators)
...
func ledgerSemanticDigest(ledger witnessLedger) string {
	return digestJSON([]any{ledger.Schema, ledger.Repository, ledger.CommitSHA, ledger.SourceSchema, ledger.PolicyDigest, ledger.RootTopologyExempt, ledger.RootREADMEExempt, ledger.Counts, ledger.SubjectWitnessDigest, ledger.MetaIndicatorDigest, ledger.IndicatorDigest, ledger.ObservationState, ledger.ClaimState, ledger.SourceObservations, ledger.SourceObservationDigest})
}
