...
jq -e '([.receipts[].claim_id] | unique | length) == 5 and ([.receipts[].predicate_id] | unique | length) == 5 and (.receipts | all(.[] as $receipt; ($receipt.program_meta_operation.authorized == true and ($receipt.claim_transitions | all(.[]; .claim_id == $receipt.claim_id and .proposition_digest == $receipt.proposition_digest and .predicate_id == $receipt.predicate_id and .evidence_digest != "")))))' "$output/report-a.json"
