...
// MeasureAll observes all supplied stages in registration order and evaluates
// each stage against its own budget.
func MeasureAll(specs []StageSpec, config Config) (Report, error) {
	if err := validateSpecs(specs); err != nil {
		return Report{}, err
	}
	report := Report{Observations: make([]Observation, 0, len(specs))}
