...
func ProjectSelfImprovementProvenanceChainPart01(uri string, version int, chain generation.SelfImprovementProvenanceChainPart01) (SelfImprovementProvenanceChainProjectionPart01, error) {
...
func (projection SelfImprovementProvenanceChainProjectionPart01) Validate() error {
	...
	if projection.Status != generation.SelfImprovementProvenanceChainBoundPart01 && projection.Status != generation.SelfImprovementProvenanceChainUnknownPart01 {
		return fmt.Errorf("unknown self-improvement provenance chain projection status %q", projection.Status)
	}
	if projection.ChainDigest == "" || projection.Digest == "" || projection.Digest != digestSelfImprovementProvenanceChainProjectionPart01(projection) {
		return fmt.Errorf("self-improvement provenance chain projection digest does not match its evidence")
	}
	return nil
}

func digestSelfImprovementProvenanceChainProjectionPart01(projection SelfImprovementProvenanceChainProjectionPart01) string {
	projection.Digest = ""
