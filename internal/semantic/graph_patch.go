...
// The graph is derived, so these digests are supplied by the owning pipeline.
...
// ValidatePatchPreconditions rejects stale or malformed edits before any
// graph mutation. Callers must perform the actual edit only after nil.
