...
func TestObserveCanonicalEntityFieldsV1(t *testing.T) {
	...
	if len(observation.Semantic.Entities) != 1 || len(observation.Semantic.Entities[0].Fields) != 1 || len(observation.Semantic.Activities) != 12 { t.Fatalf("semantic shape = %#v", observation.Semantic) }
	if len(observation.DeclarationOrder) != 13 || observation.DeclarationOrder[0] != SourceID { t.Fatalf("declaration order = %#v", observation.DeclarationOrder) }
	if observation.Semantic.Activities[0].Name != "ParseEntityFields" || observation.Semantic.Activities[11].Name != "PublishEntityFieldsReceipt" { t.Fatalf("semantic activity order = %#v", observation.Semantic.Activities) }
	...
	if !observation.GetPutRoundTrip || len(observation.SourceMap.Mappings) == 0 { t.Fatal("atomic BX or source-map evidence missing") }
	if !hasNavigationSymbol(observation.Symbols, "OrderNumber", FieldID) { t.Fatalf("field symbol missing: %#v", observation.Symbols) }
	if len(observation.StableIDs) != 14 { t.Fatalf("stable identity count = %d", len(observation.StableIDs)) }
}
