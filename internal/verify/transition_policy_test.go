...
func TestTransitionBranchPolicyIsDeterministic(t *testing.T) {
	...
	for _, valid := range [][2]string{{"agent/ci-workflow", "dev"}, {"dev", "main"}, {"agent/main-history-reconciliation-fixture-20260903", "main"}} {
		if err := CheckPullRequestPolicy(valid[0], valid[1]); err != nil {
			t.Fatalf("%s -> %s: %v", valid[0], valid[1], err)
		}
	}
	for _, invalid := range [][2]string{{"agent/ci-workflow", "main"}, {"agent/main-history-reconciliation-", "main"}, {"agent/ci-workflow", "integration"}, {"feature/work", "dev"}, {"dev", "integration"}} {
