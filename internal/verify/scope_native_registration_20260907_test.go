...
func TestNativeRegistrationCommonExecutionOwnershipIsExact(t *testing.T) {
	want := []string{
	...
	if !known || !reflect.DeepEqual(got, want) {
		t.Fatalf("native registration ownership is not exact: known=%t paths=%v", known, got)
	}
	if err := CheckPathScopeForBranch(want, "agent/registration-native-integration-20260907"); err != nil {
		t.Fatal(err)
	}
	for _, path := range []string{".github/workflows/ci.yml", ".github/foundation-authorization.json",
		".github/workflows/gooo-release-publish.yml", "internal/meta/semantic/graph.go", "go.mod",
		"internal/meta/languageassurance/verticalsliceclosureshadow/evidence/denominator-v31.json"} {
		if err := CheckPathScopeForBranch([]string{path}, "agent/registration-native-integration-20260907"); err == nil {
			t.Errorf("native registration scope allowed unrelated path %q", path)
		}
	}
}
