...
func TestEntityFieldsV1ScopeIsExplicit(t *testing.T) {
	...
	if !ok || len(paths) != 34 { t.Fatalf("EntityFields scope: known=%t paths=%d", ok, len(paths)) }
	allowed := []string{
		...
		"cmd/gooo/check_part01.go", "cmd/gooo/generate_pipeline_part04.go", "internal/entityfieldsv1/observation.go", "internal/lsp/entity_fields_references.go", "internal/meta/entityfields/evaluate.go", "internal/meta/languagereadiness/languagesyntax/corpus_paths.go", "internal/syntax/entity_fields_support.go",
	}
