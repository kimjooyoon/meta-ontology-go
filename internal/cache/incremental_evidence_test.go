...
func TestCacheSameKeyCrossProcessStampede(t *testing.T) {
	...
	for index := 0; index < 2; index++ {
		...
		compute := filepath.Join(root, fmt.Sprintf("compute-%d", index))
		...
		command.Env = append(os.Environ(),
			stampedeHelperEnv+"=1",
			stampedeRootEnv+"="+root,
			stampedeReleaseEnv+"="+release,
			stampedeReadyEnv+"="+ready,
			stampedeComputeEnv+"="+compute,
			stampedeResultEnv+"="+result,
		)
...
func runStampedeHelper(t *testing.T) {
	t.Helper()
	root := os.Getenv(stampedeRootEnv)
	key := makeTestKey(t, "v1", "billing")
	cache, err := Open(root)
	if err != nil {
		t.Fatal(err)
