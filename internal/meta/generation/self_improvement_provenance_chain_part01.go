...
func BuildSelfImprovementProvenanceChainPart01(declarationDigest, sourceDigest, semanticIRDigest, graphDigest, generatedDigest, reverseObservationDigest string) SelfImprovementProvenanceChainPart01 {
	values := []struct { name, digest string }{
	...
	for _, value := range values {
		bound := value.digest != ""
		stages = append(stages, SelfImprovementProvenanceStagePart01{Name: value.name, Digest: value.digest, Bound: bound})
...
func (chain SelfImprovementProvenanceChainPart01) Validate() error {
	...
	expectedNames := []string{"gooo-declaration", "source", "semantic-ir", "semantic-graph", "generated", "reverse-observation"}
	expectedDigests := []string{chain.DeclarationDigest, chain.SourceDigest, chain.SemanticIRDigest, chain.GraphDigest, chain.GeneratedDigest, chain.ReverseObservationDigest}
	...
	if chain.Status != SelfImprovementProvenanceChainBoundPart01 && chain.Status != SelfImprovementProvenanceChainUnknownPart01 {
		return fmt.Errorf("unknown self-improvement provenance chain status %q", chain.Status)
	}
	if chain.Status == SelfImprovementProvenanceChainBoundPart01 && chain.CausalReason != SelfImprovementProvenanceChainCompleteReasonPart01 {
		return fmt.Errorf("bound self-improvement provenance chain has unexpected reason %q", chain.CausalReason)
	}
	if chain.Status == SelfImprovementProvenanceChainUnknownPart01 && !strings.HasPrefix(chain.CausalReason, "MISSING_") {
...
func normalizeSelfImprovementProvenanceDigestPart01(value string) string { return strings.TrimSpace(value) }
