...
func causalFailureIndex(failures []generation.ObservationFailure) (map[string]generation.ObservationFailure, error) {
	...
	for _, failure := range failures {
		if failure.ActionIndicatorID == "" || failure.Decision != string(generation.ReceiptDecisionRefuted) ||
			...
			return nil, fmt.Errorf("causal failure context is incomplete")
		}
		if _, exists := index[failure.ActionIndicatorID]; exists {
			return nil, fmt.Errorf("causal failure context is duplicated")
		}
...
func validateMetricCausalUnknown(unknown generation.ReceiptUnknown) error {
	if unknown.ActionIndicatorID == "" || unknown.RequiredIndicatorID == "" || unknown.Operation == "" ||
		...
		return fmt.Errorf("causal unknown context is incomplete")
	}
	return nil
}

func metricCausalUnknownRecord(unknown generation.ReceiptUnknown) transformationeffect.CausalUnknownRecord {
