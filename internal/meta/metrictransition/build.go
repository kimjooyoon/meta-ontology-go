...
func Build(options Options) (Result, error) {
	...
	effect, err := buildEffectEvidence(inputs)
	if err != nil {
		return Result{}, err
	}
	before := StateReference{Schema: StateSchema, Digest: state.Digest, CommitSHA: options.ExpectedSHA, Materialization: "MEASURED"}
	after := StateReference{Schema: StateSchema, Digest: state.Digest, CommitSHA: options.ExpectedSHA, Materialization: "ALIAS_OF_BEFORE", BasisDigest: effect.Artifacts[0].Digest}
	ledger := TransitionLedger{
		Schema: LedgerSchema, Status: "BOUND", Decision: "FIXED_POINT_ZERO_DELTA", Reason: "VERIFIED_EFFECT_TREE_IDENTITY",
...
func transitionIndicators(state RepositoryState, effect EffectEvidence, sha string) []Indicator {
	return []Indicator{
		...
		{ID: "coherence.fixed-point-alias", Family: "coherence", ProofChoice: "COHERENCE", Satisfied: true, MetaOperation: "derive-after-from-tree-identity", EvidenceDigest: effect.Artifacts[0].Digest},
