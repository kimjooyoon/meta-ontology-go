...
func TestArtifactBindsCatalogToMetaCodeContent(t *testing.T) {
	...
	if artifact.Report.Summary.Concepts != 29 || artifact.Bindings.Paths != 116 {
		t.Fatalf("summary=%#v bindings=%#v", artifact.Report.Summary, artifact.Bindings)
	}
	replay := BuildArtifact(repository)
	if artifact.Bindings != replay.Bindings {
		t.Fatalf("binding replay=%#v want %#v", replay.Bindings, artifact.Bindings)
	}
	if artifact.Bindings.Bytes == 0 || artifact.RepositoryWrites != 0 {
