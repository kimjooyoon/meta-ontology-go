...
func (s *inspection) indicators() []Indicator {
	...
	return []Indicator{
		...
		metric("functional.user-journeys", "OUTCOME", "COHERENCE", "execute-user-positive-paths", int64(s.journeysPassed), 7),
		metric("resource.envelopes", "OUTCOME", "COHERENCE", "reduce-runner-resource-envelopes", int64(s.envelopesPassed), 7),
		metric("profile.samples", "DRIVER", "FOUNDATION", "observe-resource-samples", int64(s.samplesObserved), int64(expectedSamples)),
		metric("profile.output-replay", "DRIVER", "REGRESSION", "compare-user-visible-output", int64(s.outputReplays), 7),
		...
		metric("functional.declared-commands", "DRIVER", "FOUNDATION", "observe-cli-surface", int64(s.upstream.Summary.DeclaredCommands), 14),
		metric("binding.binary", "DRIVER", "FOUNDATION", "bind-profiled-executable", boolInt(s.binaryBound && s.sourceBound), 1),
		metric("binding.meta-operations", "DRIVER", "FOUNDATION", "join-functional-and-resource-meta-operations", int64(s.metaBindings), 19),
