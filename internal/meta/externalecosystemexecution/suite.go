...
func RunSuite() SuiteReport {
	...
	for _, name := range names {
		...
		if name != "observation-unavailable" { value := exactObservation(); observation = &value; mutateCase(name, observation) }
		report := Evaluate(observation)
		wantDecision, wantResolution := DecisionFailClosed, "EXACT"
		if name == "exact" { wantDecision = DecisionConfirmed }
		if name == "observation-unavailable" || name == "reference-unavailable" || name == "reference-decision-unknown" { wantResolution = "COARSE" }
		passed := report.Decision == wantDecision && report.Resolution == wantResolution
		if passed { suite.Passed++ } else { suite.Unresolved++ }
...
func mutateCase(name string, o *Observation) {
	switch name {
	case "reference-unavailable": o.Reference.Available = false
	case "reference-decision-unknown": o.Reference.Decision = "UNRECOGNIZED"
	case "commit-mismatch": o.Reference.Commit = "different"
	case "go-version-mismatch": o.GoVersion = "go1.26.0"
	case "execution-failure": o.Runs[1].Passed, o.Runs[1].ExitCode = false, 1
	case "replay-mismatch": o.Runs[1].NormalizedSHA256 = Digest("different")
	case "source-write": o.SourceAfter.Dirty = true
	case "unknown-event": o.Runs[0].Passed, o.Runs[0].UnknownEvents = false, []string{"future-action"}
	}
}
