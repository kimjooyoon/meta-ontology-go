...
func writeStatus(o *Observation) (string, string) {
	...
	for _, state := range states {
		if !state.Available {
			return "UNKNOWN", "REPOSITORY_STATE_UNAVAILABLE"
		}
	}
	if o.SourceBefore.Dirty || o.SourceAfter.Dirty || o.ExternalBefore.Dirty || o.ExternalAfter.Dirty ||
		...
		o.OfficialMutationCount != 0 || o.PromotionCount != 0 {
		return "UNSATISFIED", "REPOSITORY_WRITE_BOUNDARY_VIOLATED"
	}
	return "SATISFIED", "REPOSITORY_WRITE_BOUNDARY_EXACT"
}

func repositoryWrites(o *Observation) int {
	status, _ := writeStatus(o)
	if status == "SATISFIED" {
		return 0
	}
	return 1
...
func proofs(o *Observation, indicators []Indicator) []Proof {
	...
	status := "UNKNOWN"
	if o != nil && o.Regression.Total > 0 {
		status = "UNSATISFIED"
		if o.Regression.Passed == o.Regression.Total && o.Regression.Unresolved == 0 {
			status = "SATISFIED"
		}
	}
