...
func buildIndicators(o *Observation) []Indicator {
	...
	if o == nil {
		...
		for i := range c { result[i] = indicator(c[i], "UNKNOWN", "EXECUTION_OBSERVATION_UNAVAILABLE") }
		return result
	}
	...
	result := make([]Indicator, len(c)); for i := range c { result[i] = indicator(c[i], values[i][0], values[i][1]) }
	return result
}

func referenceStatus(r ReferenceReceipt) (string, string) {
	if !r.Available { return "UNKNOWN", "REFERENCE_EVIDENCE_UNAVAILABLE" }
	if r.Decision != ExpectedReferenceDecision { return "UNKNOWN", "REFERENCE_DECISION_UNKNOWN" }
	if !r.BindingExact || r.ContractVersion != ReferenceContractVersion || r.Resolution != "EXACT" ||
		r.URL != ExpectedReferenceURL || r.ModuleGo != ExpectedModuleGo { return "UNSATISFIED", "REFERENCE_BINDING_MISMATCH" }
	return "SATISFIED", "REFERENCE_BINDING_EXACT"
}

func exactValue(available bool, actual, expected, name string) (string, string) {
	if !available || actual == "" { return "UNKNOWN", name + "_UNAVAILABLE" }
	if actual != expected { return "UNSATISFIED", name + "_MISMATCH" }
	return "SATISFIED", name + "_EXACT"
}

func runStatus(runs []RunObservation, index int) (string, string) {
	if len(runs) <= index { return "UNKNOWN", "EXTERNAL_RUN_UNAVAILABLE" }
	if len(runs[index].UnknownEvents) > 0 { return "UNSATISFIED", "EXTERNAL_EVENT_UNSUPPORTED" }
	if !runs[index].Passed || runs[index].ExitCode != 0 { return "UNSATISFIED", "EXTERNAL_RUN_FAILED" }
	return "SATISFIED", "EXTERNAL_RUN_PASSED"
}

func replayStatus(runs []RunObservation) (string, string) {
	if len(runs) < 2 || runs[0].NormalizedSHA256 == "" || runs[1].NormalizedSHA256 == "" { return "UNKNOWN", "NORMALIZED_REPLAY_UNAVAILABLE" }
	if runs[0].NormalizedSHA256 != runs[1].NormalizedSHA256 { return "UNSATISFIED", "NORMALIZED_REPLAY_MISMATCH" }
