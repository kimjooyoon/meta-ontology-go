...
func TestEvaluateClosesFixedUserUseCase(t *testing.T) {
	...
	if report.Decision != "PASS" || report.Resolution != "EXACT" || report.Summary.Coordinates.Satisfied != 8 || report.Summary.Coordinates.Total != 8 || report.Summary.UserDecisions != 2 || report.Summary.GeneratedInstances != 1 || report.Summary.GoldenMatches != 1 || report.PromotionCreditBPS != 0 {
...
func TestEvaluateGeneratedInvocationLinkMismatchPreservesInvariant(t *testing.T) {
	input := testInput()
	input.Observation.GeneratedDigest = digest('9')
	report, err := Evaluate(input)
	if err != nil {
		t.Fatal(err)
	}
...
func testInput() Input {
	artifactDigest, schemaDigest, toolDigest, generatedDigest := digest('a'), digest('b'), digest('c'), digest('e')
	return Input{SubjectSHA: string(repeat('1', 40)), Contract: CanonicalContract(), ProducerReceipt: ProducerReceipt{Schema: "gooo/symbolic-invocation-schema-receipt/v1", Decision: "PASS", Resolution: "EXACT", Reason: "EXTERNAL_SCHEMA_VALIDATION_OBSERVED", SubjectSHA: string(repeat('1', 40)), Compiler: CompilerEvidence{GoVersion: "1.27.0", BinaryDigest: digest('d'), BinaryBytes: 12492193, RegisteredEmitters: 3}, Source: SourceCoordinate{GoooFiles: 3, GoFiles: 0, GoooLines: 16, Files: 6, Directories: 0}, Artifact: ArtifactEvidence{Kind: "symbolic-invocation-schema", ArtifactSchema: "gooo/symbolic-invocation-schema-artifact/v1", Digest: artifactDigest, JSONSchemaDialect: "https://json-schema.org/draft/2020-12/schema", JSONSchemaDigest: schemaDigest}, Validation: ValidationEvidence{Tool: "github.com/santhosh-tekuri/jsonschema/cmd/jv@v0.7.0", ToolDigest: toolDigest, AcceptedInstances: 1, RejectedInstances: 1, GeneratedCount: 1, GoldenMatches: 1, GeneratedDigest: generatedDigest}, DeterministicReplays: 1, Resources: ResourceEvidence{Samples: []ResourceSample{{1, 5, 10000}, {2, 6, 10001}, {3, 5, 9999}, {4, 4, 9998}, {5, 5, 9997}}, SampleCount: 5, MaxWallMS: 6, MaxRSSKiB: 10001}, Effects: Effects{}, NotClaimed: CanonicalNonClaims()}, ProducerArtifact: ProducerArtifact{Schema: "gooo/symbolic-invocation-schema-artifact/v1", Decision: "PASS", Resolution: "SYMBOLIC_ONLY", Reason: "SYMBOLIC_INVOCATION_SCHEMA_EMITTED", Kind: "symbolic-invocation-schema", Extensions: ArtifactExtensions{RegisteredEmitters: 3, Kinds: []string{"operation-interface", "operation-manifest", "symbolic-invocation-schema"}}, Effects: Effects{}, Digest: artifactDigest}, Observation: Observation{Schema: "gooo/symbolic-invocation-usecase-observation/v1", Decision: "PASS", Resolution: "EXACT", Reason: "EXTERNAL_USER_VALIDATION_REPLAYED", SubjectSHA: string(repeat('1', 40)), ArtifactDigest: artifactDigest, JSONSchemaDigest: schemaDigest, ToolDigest: toolDigest, AcceptedInstances: 1, RejectedInstances: 1, GeneratedCount: 1, GoldenMatches: 1, GeneratedDigest: generatedDigest, Effects: Effects{}}}
}
