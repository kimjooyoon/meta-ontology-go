...
func Generate(input Input) (Artifact, error) {
	...
	claims := []ClaimStatement{
		...
		{ID: "case-envelope-policy-bound", Proposition: "case-envelope-policy-matches-source", TargetDigest: sourceDigest, Dependencies: []string{"source-bytes-bound"}, ProofChoice: "FOUNDATION", MetaOperation: "bind-case-envelope-policy", Coordinate: Coordinate{"GENERATE", "policy-evidence", "CASE_ENVELOPE_POLICY_SOURCE_CARRIED"}, EvidenceDigest: []string{evidence[0].EvidenceDigest}, State: "CARRIED"},
		{ID: "consumer-authority", Proposition: "verified-consumer-may-read-only-consume", TargetDigest: "READ_ONLY_CONSUMPTION", Dependencies: []string{"source-bytes-bound", "operation-receipt-bound", "no-byte-authority", "recipe-match", "case-envelope-policy-bound"}, ProofChoice: "COHERENCE", MetaOperation: "grant-read-only-consumption", Coordinate: Coordinate{"GENERATE", "authority-evidence", "AUTHORITY_REQUIRES_INDEPENDENT_RECHECK"}, EvidenceDigest: []string{evidence[0].EvidenceDigest, evidence[1].EvidenceDigest, evidence[2].EvidenceDigest}, State: "NONE"},
	}
