...
import (
	...
	eligibility "github.com/kimjooyoon/meta-ontology-go/internal/meta/languageassurance/changedsurfacereceipteligibility"
)
...
func validAssurance(report assuranceReport) bool {
	...
	s := report.Summary
	if s.DenominatorTotal != 12 || s.Operating != 8 || s.NotImplemented != 4 || s.ImplementationCoverageBPS != 6666 || s.UnresolvedIndicators != 0 || s.ViolatedGuardrails != 0 || s.RepositoryWrites != 0 {
		return false
	}
	matches := 0
...
func validEligibility(report eligibility.Report) bool {
	...
	return s.DenominatorTotal == 12 && s.BeforeOperating == 8 && s.AfterOperating == 9 && s.BeforeCoverageBPS == 6666 && s.AfterCoverageBPS == 7500 && s.CapsulesTotal == 3 && s.CapsulesExact == 3 && s.CapsuleCoverageBPS == 10000 && s.ShadowCasesTotal == 6 && s.ShadowCasesPassed == 6 && s.MetaOperationsRequired == 1 && s.MetaOperationsObserved == 1 && s.MetaOperationCoverageBPS == 10000 && s.EligiblePaths == 1 && s.UnknownPaths == 0 && s.BlockedPaths == 0 && len(report.MetaOperations) == 6 && validEligibilityIndicators(report.Indicators)
}
