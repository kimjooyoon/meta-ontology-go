...
func validateEvidence(input Input) (eligibilityReport, int, int, int, string, string) {
	...
	rawExact := 0
	for _, binding := range bindings { if binding.Exact { rawExact++ } }
...
func validAssurance(report assuranceReport) bool {
	if report.Schema != "gooo/language-assurance-report/v1" || report.SubjectSHA != PredecessorSHA ||
		...
		report.CandidateResolution != ResolutionExact || report.ReportDigest != AssuranceReportHash { return false }
	s := report.Summary
	if s.DenominatorTotal != 12 || s.Operating != 10 || s.NotImplemented != 2 || s.ImplementationCoverageBPS != 8333 ||
		s.UnresolvedIndicators != 0 || s.ViolatedGuardrails != 0 || s.RepositoryWrites != 0 || len(report.Obligations) != 12 { return false }
	matches := 0
	for _, item := range report.Obligations {
		if item.MetricID == MetricID && item.Status == "NOT_IMPLEMENTED" && item.Resolution == "NONE" && item.MetaOperation == "" { matches++ }
	}
	return matches == 1
}
