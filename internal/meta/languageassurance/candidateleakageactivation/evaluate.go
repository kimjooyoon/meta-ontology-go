...
func Evaluate(input Input) Receipt {
	...
	applied := reason == ""
	decision, transitionApplied, blocked, after := DecisionFailClosed, 0, 1, 7
	if applied {
		decision, reason, resolution = DecisionApplied, ReasonApplied, ResolutionExact
		transitionApplied, blocked, after = 1, 0, 8
	}
	unknown := 0
	...
	summary := Summary{DenominatorTotal: 12, BeforeOperating: 7, AfterOperating: after,
		BeforeCoverageBPS: 5833, AfterCoverageBPS: after * 10000 / 12,
