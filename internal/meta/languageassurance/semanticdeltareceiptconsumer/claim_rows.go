...
func unknownLedger(stage, step, reason string) ([]Claim, []ClaimTransition) {
	...
	return []Claim{claim}, []ClaimTransition{{ClaimID: boundedClaimID, Kind: claimKindBounded, FromStatus: statusOpen, ToStatus: statusOpen, Stage: stage, Step: step, Reason: reason}}
}

func boundedClaim(afterDigest, status string) Claim {
	return Claim{ID: boundedClaimID, Kind: claimKindBounded, Subject: "source-pair", Predicate: "bounded-semantic-equivalence", Object: afterDigest, Status: status, Stage: "adjudicate", Step: "bounded-equivalence", Reason: "BOUNDED_EQUIVALENCE_LEDGER", PropositionDigest: propositionDigest(claimKindBounded, "source-pair", "bounded-semantic-equivalence", "after")}
}

func preservationClaim(before Claim, afterObject, status, reason string) Claim {
	return Claim{ID: preservationClaimID(before), Kind: claimKindPreserve, Subject: before.ID, Predicate: "preserves", Object: afterObject, Status: status, Stage: "adjudicate", Step: "before-claim-preservation", Reason: reason, PropositionDigest: propositionDigest(claimKindPreserve, before.ID, "preserves", before.PropositionDigest), PreservationOf: before.ID}
}
