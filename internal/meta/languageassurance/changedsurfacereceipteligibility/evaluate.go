...
func Evaluate(input Input) Report {
	...
	report := Report{
		...
		Summary: Summary{DenominatorTotal: 12, BeforeOperating: 8, AfterOperating: 8,
			BeforeCoverageBPS: 6666, AfterCoverageBPS: 6666, CapsulesTotal: 3, BlockedPaths: 1},
	}
	...
	if len(input.Assurance.Payload) == 0 || len(input.ShadowReport.Payload) == 0 ||
		len(input.ShadowSuite.Payload) == 0 {
		report.Summary.UnknownPaths = 1
		return finish(report)
	}
	...
	shadowReport, reportErr := decode[shadowReportCapsule](input.ShadowReport.Payload)
	shadowSuite, suiteErr := decode[shadowSuiteCapsule](input.ShadowSuite.Payload)
	if assuranceErr != nil || reportErr != nil || suiteErr != nil ||
		!validSemantics(assurance, shadowReport, shadowSuite) {
	...
	report.Summary = Summary{DenominatorTotal: 12, BeforeOperating: 8, AfterOperating: 9,
		BeforeCoverageBPS: 6666, AfterCoverageBPS: 7500,
		CapsulesTotal: 3, CapsulesExact: 3, CapsuleCoverageBPS: 10_000,
		ShadowCasesTotal: 6, ShadowCasesPassed: 6,
