...
func TestAssuranceDecisionMatrix(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if report.AssuranceDecision != AssurancePartial || report.CandidateDecision != test.decision || report.CandidateReason != test.reason || report.Summary.Operating != 10 || report.Summary.DenominatorTotal != 12 || report.Summary.ImplementationCoverageBPS != 8333 {
