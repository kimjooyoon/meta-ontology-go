...
func toolchainFixture(head string) []byte {
	surfaces := []any{
		surfaceFixture("language-syntax-roundtrip", languagesyntax.ReportSchema, head, languagesyntax.FixedCapabilityTotal),
	...
	return fixtureJSON(map[string]any{"schema": "gooo/toolchain-conformance-report/v1",
		...
		"summary": map[string]any{"surfaces_satisfied": 9, "surfaces_total": 9,
			"cases_satisfied": toolchainconformance.ExpectedCaseCount, "cases_total": toolchainconformance.ExpectedCaseCount, "executed_cases": toolchainconformance.ExpectedCaseCount,
