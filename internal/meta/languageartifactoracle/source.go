...
func projectSource(raw []byte, selectedEntry string) (projection, error) {
	...
	for number, rawLine := range strings.Split(string(raw), "\n") {
		...
		case strings.HasPrefix(line, "activity "):
			...
			if err != nil { return projection{}, err }
			if name == selectedEntry { result.Activity, inputNames, outputName = name, inputs, output }
		default:
	...
	for index, name := range inputNames {
		...
		if !ok { return projection{}, fmt.Errorf("input entity %q missing", name) }
		result.Inputs[index] = binding
	}
	output, ok := entities[outputName]
	if !ok { return projection{}, fmt.Errorf("output entity %q missing", outputName) }
