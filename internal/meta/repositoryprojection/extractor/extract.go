...
func Extract(root, logical string) (map[string][]byte, []string, error) {
	...
	var chosen []declaration; var output rendered
	for _, item := range ordered { chosen = append(chosen, item); candidate, renderErr := render(fset, file, source, chosen, list); if renderErr != nil { return nil, nil, renderErr }; if bytes.Count(candidate.source, []byte{'\n'}) <= 75 { output = candidate; break } }
	if len(output.source) == 0 { return nil, nil, fail("derive-recipe", "select-declaration", "NO_SAFE_CAPACITY", "KNOWN_CONTRADICTION", "report-contradiction", nil) }
