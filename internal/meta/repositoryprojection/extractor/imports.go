...
func imports(file *ast.File) ([]importSpec, error) {
	...
	for _, node := range file.Decls {
		...
		if !ok || group.Tok.String() != "import" { continue }
...
func validateImports(root string, list []importSpec) error {
	...
	for _, item := range list {
		if item.name == "." { return fail("validate-ast-imports", "resolve-import", "UNSUPPORTED_DOT_IMPORT", "KNOWN_CONTRADICTION", "report-contradiction", []string{item.path}) }
		if module != "" && (item.path == module || strings.HasPrefix(item.path, module+"/")) {
			local := root
			if item.path != module { local = filepath.Join(root, filepath.FromSlash(strings.TrimPrefix(item.path, module+"/"))) }
			if !hasGoFile(local) { return fail("validate-ast-imports", "resolve-import", "UNRESOLVED_IMPORT", "DIRECT_MISSING", "restore-import-package", nil) }
			continue
		}
...
func moduleName(root string) string {
	data, err := os.ReadFile(filepath.Join(root, "go.mod")); if err != nil { return "" }
	for _, line := range strings.Split(string(data), "\n") { fields := strings.Fields(line); if len(fields) == 2 && fields[0] == "module" { return fields[1] } }
	return ""
}

func importName(item importSpec) string {
	if item.name != "" && item.name != "_" { return item.name }
	base := pathpkg.Base(item.path)
	if strings.HasPrefix(base, "v") && len(base) > 1 && base[1] >= '0' && base[1] <= '9' { base = pathpkg.Base(pathpkg.Dir(item.path)) }
	return base
}
