...
func safePath(root, logical string) (string, error) {
	if logical == "" || filepath.IsAbs(logical) || strings.ContainsRune(logical, 0) { return "", fmt.Errorf("unsafe extraction path %q", logical) }
	clean := filepath.Clean(filepath.FromSlash(logical))
	if clean == "." || clean == ".." || strings.HasPrefix(clean, ".."+string(filepath.Separator)) { return "", fmt.Errorf("unsafe extraction path %q", logical) }
...
func validateBuildHeader(header []byte) error {
	...
	for scanner.Scan() {
		line := strings.TrimSpace(scanner.Text()); if !strings.HasPrefix(line, "//go:build ") && !strings.HasPrefix(line, "// +build ") { continue }
		expr, err := constraint.Parse(line); if err != nil { return fail("validate-ast", "build-constraints", "BUILD_TAG_CONFLICT", "KNOWN_CONTRADICTION", "report-contradiction", nil) }
		if strings.HasPrefix(line, "//go:build ") { goExpr = expr.String() } else if legacyExpr == "" { legacyExpr = expr.String() } else { legacyExpr += " && " + expr.String() }
	}
	if goExpr != "" && legacyExpr != "" && goExpr != legacyExpr { return fail("validate-ast", "build-constraints", "BUILD_TAG_CONFLICT", "KNOWN_CONTRADICTION", "report-contradiction", nil) }
	return nil
}
