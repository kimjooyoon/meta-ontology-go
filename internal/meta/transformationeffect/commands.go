...
func runAction(box *sandbox, opts Options, plan generation.Plan, action generation.Action, check bool) ([]byte, error) {
	...
	output, err := runCombined(box.root, os.Environ(), "go", args...)
	if err != nil {
...
func freshMetrics(box *sandbox, expected string) (linecaps.LineMetricsReport, []byte, error) {
