...
func buildIndicators(source Source, observation Observation, effects []Effect, unknown Unknown, claim ClaimTransition, decision string) []Indicator {
	...
	return []Indicator{
		...
		indicator("OEL-OBS-02", "OBSERVATION", "observe-repository", "observer-effect-judge", "observe-repository", "FOUNDATION", "before snapshot exists", observation.RepositoryStorage.BeforeDigest != "", status(observation.RepositoryStorage.BeforeDigest != "", false), unknown),
		indicator("OEL-OBS-03", "OBSERVATION", "observe-repository", "observer-effect-judge", "observe-repository", "COHERENCE", "after snapshot exists", observation.RepositoryStorage.AfterDigest != "", status(observation.RepositoryStorage.AfterDigest != "", false), unknown),
		...
		indicator("OEL-OBS-06", "OBSERVATION", "separate-observation", "observer-effect-judge", "separate-observation-from-effect", "COHERENCE", "observation and effects are distinct", len(effects) == 4, status(len(effects) == 4, false), unknown),
