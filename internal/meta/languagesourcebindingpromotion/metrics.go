...
func indicators(report Report) []Indicator {
	...
	return []Indicator{
		...
		metric("dependency-blocked", "DRIVER", "COHERENCE", "propagate-blocked-promotion", s.DependencyBlocked, 3),
...
func proofs(report Report) []Proof {
	return []Proof{
		...
		{Choice: "COHERENCE", MetaOperation: "traverse-source-binding-promotion", EvidenceDigest: digestJSON(report.Cases),
			Passed: report.Summary.ExactPromotions == 1 && report.Summary.ExactClaims == 3 && report.Summary.PolicyReplays == 1},
