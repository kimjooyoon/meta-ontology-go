...
func userObligations() []Obligation {
	return []Obligation{
		...
		obligation("USER-RUN-SOURCE", AudienceUser, ClassOutcome, "execute a Gooo source program", missing(), "require-source-execution-receipt", ProofFoundation),
		obligation("USER-DETERMINISTIC-RUNTIME-RESULT", AudienceUser, ClassOutcome, "observe a deterministic runtime result", missing(), "require-runtime-result-replay", ProofCoherence),
		obligation("USER-RUNTIME-FAILURE-DIAGNOSTIC", AudienceUser, ClassOutcome, "observe a fail-closed runtime error", missing(), "require-runtime-failure-receipt", ProofRegression),
