...
func observeOperation(operation actionability.OperationWitness, binding ArtifactBinding,
	...
	for _, artifact := range document.Artifacts {
		if slices.Contains(artifact.EvidenceKeys, binding.EvidenceKey) {
	...
	if len(matches) != 1 {
		if len(matches) == 0 {
			witness.Status = "ARTIFACT_MISSING"
		} else {
			witness.Status = "ARTIFACT_AMBIGUOUS"
		}
		return witness
	}
	artifact := matches[0]
	witness.ExactHead = artifact.Name == witness.ExpectedArtifact && artifact.HeadSHA == document.CommitSHA
	witness.DigestBound = validDigest(artifact.Digest)
	witness.ReplayBound = validDigest(artifact.ReplayDigest)
	witness.Canonical = witness.ExactHead && witness.DigestBound && witness.ReplayBound
	switch {
	case !witness.ExactHead:
		witness.Status = "EXACT_HEAD_MISMATCH"
	case !witness.DigestBound:
		witness.Status = "DIGEST_UNBOUND"
	case !witness.ReplayBound:
		witness.Status = "REPLAY_UNBOUND"
	default:
