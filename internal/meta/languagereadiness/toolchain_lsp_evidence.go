...
func validateToolchainLSP(reports []metalsp.Report, expectedHeadSHA string) (string, error) {
	if len(reports) != 1 {
		return "", fmt.Errorf("FAIL_CLOSED: toolchain LSP evidence is not unique")
	}
	report := reports[0]
	if err := metalsp.Validate(report, expectedHeadSHA); err != nil {
		return "", fmt.Errorf("verify toolchain LSP: %w", err)
	}
