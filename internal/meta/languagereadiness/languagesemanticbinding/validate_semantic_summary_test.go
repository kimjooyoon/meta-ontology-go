...
func TestVersionedDenominatorsMatchActivatedCorpus(t *testing.T) {
	if syntaxCaseDenominator != 29 || syntaxValidSourceDenominator != 26 || syntaxInvalidCaseDenominator != 3 || syntaxGoooLineDenominator != 413 {
		t.Fatal("syntax denominator must match the activated 29-case, 26-source, 413-line corpus")
	}
