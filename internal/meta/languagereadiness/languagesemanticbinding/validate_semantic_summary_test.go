...
func TestVersionedDenominatorsMatchActivatedCorpus(t *testing.T) {
	if syntaxCaseDenominator != 31 || syntaxValidSourceDenominator != 28 || syntaxInvalidCaseDenominator != 3 || syntaxGoooLineDenominator != 457 {
		t.Fatal("syntax denominator must match the activated 31-case, 28-source, 457-line corpus")
	}
	if semanticCaseDenominator != 33 || semanticSourceDenominator != 28 {
		t.Fatal("semantic denominator must match the activated 33-case, 28-source corpus")
	}
}
