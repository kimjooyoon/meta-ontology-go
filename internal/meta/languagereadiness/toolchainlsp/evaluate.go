...
func Evaluate(headSHA string, corpus Corpus, concept ConceptBinding) Report {
	...
	if !validSHA(headSHA) {
		report.Reason = "TOOLCHAIN_LSP_HEAD_UNKNOWN"
		report.Summary.HeadMismatches = 1
		return finish(report)
	}
	if err := ValidateCorpus(corpus); err != nil {
		report.Reason = "TOOLCHAIN_LSP_CORPUS_DRIFT"
		report.Summary.CorpusDrift = 1
		return finish(report)
	}
	if reason, err := validateConcept(concept); err != nil {
		report.Reason = reason
		report.Summary.ConceptDrift = 1
		return finish(report)
	}
	observations, stats, err := evaluateRuntime()
	if err != nil {
		report.Reason = "TOOLCHAIN_LSP_RUNTIME_UNRESOLVED"
	...
	if report.Summary.CasesSatisfied == len(caseContract) && report.Summary.ProofFailures == 0 {
		...
		report.Reason = "TOOLCHAIN_LSP_CASES_UNRESOLVED"
	}
