...
func indicators(summary Summary) []Indicator {
	return []Indicator{
		...
		newFloorIndicator("gooo.metric.language.deterministic-query-readiness-completed.v1", "DRIVER", "COHERENCE", "bind-readiness-floor", summary.ReadinessCompleted, 16),
...
func newFloorIndicator(metricID, class, proofChoice, operation string, value, target int) Indicator {
	result := newIndicator(metricID, class, proofChoice, operation, value, target)
	result.Satisfied = value >= target
	return result
}
