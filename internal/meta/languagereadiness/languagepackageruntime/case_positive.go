...
func executePositive(definition Definition, baseline packageruntime.Result) CaseResult {
	...
	if result.Satisfied {
		result.Reason = "RUNTIME_OBSERVATION_EXACT"
	} else {
		result.Reason = "RUNTIME_OBSERVATION_MISMATCH"
	}
	return result
}
...
func positiveAssertion(assertion string, observed, baseline packageruntime.Result) bool {
	switch assertion {
	case "PACKAGE_GRAPH":
		return len(observed.Image.Packages) == 4
	case "DIAMOND_ORDER":
		return strings.Join(observed.Image.InitOrder, ",") ==
			"example/core,example/left,example/right,example/app"
	case "MULTI_SOURCE":
		return packageSources(observed, "example/app") == 2
	case "ENTRY_CONTRACT":
		return observed.Image.Entry.Activity == "Run" &&
	...
	case "CANONICAL_REPLAY":
		return observed.ResultDigest != ""
	case "SEMANTIC_BINDING":
		return semanticBindings(observed) == 5
	case "ZERO_EFFECTS":
		return observed.Effects == 0 && observed.RepositoryWrites == 0 && !observed.MutationAuthorized
	}
	return false
}
