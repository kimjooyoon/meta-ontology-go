...
func classifyGuardrail(summary *Summary, definition Definition, result CaseResult) {
	if result.Observed == "UNKNOWN_FAILURE" {
		summary.UnknownObservations++
	}
	if result.Observed != "ACCEPTED" {
		return
	}
	summary.InvalidAcceptances++
	switch definition.Mutation {
	case "DUPLICATE_PACKAGE", "UNKNOWN_IMPORT", "IMPORT_CYCLE":
		summary.GraphAcceptances++
	case "HEADER_MISMATCH", "PARSE_ERROR":
		summary.SourceAcceptances++
	case "UNKNOWN_ENTRY_PACKAGE", "UNKNOWN_ENTRY_ACTIVITY":
		summary.EntryAcceptances++
	}
}
