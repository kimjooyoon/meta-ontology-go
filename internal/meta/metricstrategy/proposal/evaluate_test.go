...
func TestConformancePlanProvesThreeExactCoordinates(t *testing.T) {
	...
	for _, coordinate := range coordinates {
		if coordinate.Status != "SATISFIED" || coordinate.EvidenceDigest == "" {
			t.Fatalf("unproven coordinate: %+v", coordinate)
		}
	}
}
...
func TestFixedRegistryAndUnknownResolution(t *testing.T) {
	if len(Registry()) != 8 {
		t.Fatalf("coordinate denominator changed: %d", len(Registry()))
	}
	coordinates := make([]Coordinate, len(registry))
	for index, spec := range registry {
		coordinates[index] = Coordinate{CoordinateSpec: spec, Status: "SATISFIED", EvidenceDigest: "sha256:evidence"}
	}
