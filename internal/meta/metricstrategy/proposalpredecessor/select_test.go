...
func TestSelectsOneExactMergedProposal(t *testing.T) {
	...
	collection := Collection{ObservedRuns: 1, ExactRuns: 1, ObservedArtifacts: 4, ExactArtifacts: 1, Candidates: []Candidate{candidate}}
...
func TestAmbiguityFailsClosed(t *testing.T) {
	...
	collection := Collection{ObservedRuns: 2, ExactRuns: 2, ObservedArtifacts: 2, ExactArtifacts: 2, Candidates: []Candidate{readyCandidate(predecessor, 1), readyCandidate(predecessor, 2)}}
