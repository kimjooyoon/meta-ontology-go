...
func validateDeltaHashes(delta BXDeltaEvidence) error {
	...
	if delta.EvidenceHash != digest(spanSetCanonical(delta.EvidenceSpans.IDs, delta.EvidenceSpans.FactKeys, delta.EvidenceSpans.Spans)) {
...
func validateCanonicalDelta(delta BXDeltaEvidence) error {
	...
	expected := canonicalDeltaEvidence{
		...
		EvidenceSpans: spanTexts(delta.EvidenceSpans.Spans), EvidenceHash: delta.EvidenceHash,
