...
func Evaluate(input []byte, expectedMode string) (Receipt, error) {
	...
	receipt := Receipt{
		...
		Subject: Subject{
			...
			SemanticIRDigest:       report.CoreIRFingerprint,
			BindingStatus:          "PARTIAL_UNKNOWN",
			MissingBindingEvidence: bindingEvidence(report.SourceDigest, report.CoreIRFingerprint),
		},
		Graph:       graph,
		Metrics:     metrics,
