	...
	return []Proof{
		{
			...
			MetaOperation: "bind-catalog-baseline", EvidenceDigest: report.SourceDigest,
			Passed: report.ActivitiesObserved == 2 && report.Improvement.Before == coordinate(0, 1),
		},
		{
			...
			Passed: extension && report.ExtensionCoreProgram == "int.add:2" && report.Summary.CoreFingerprintSensitive == coordinate(1, 1),
		},
		{
			...
			Passed: report.Baseline.Passed == 3 && report.Summary.UnknownCounterexamplePassed && (!extension || report.Extension.Passed == 3),
		},
	}
}
