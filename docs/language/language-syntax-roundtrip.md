...
The `v2` registry contains exactly 33 cases: 30 valid `.gooo` sources and three
invalid fixtures. CI also walks the repository and requires the observed
`.gooo` path set to equal the 30 registered valid paths plus five package
members. The current fixed corpus contains exactly 449 physical Gooo lines, and each file carries its
...
| COHERENCE | replay-ast-bytes-semantics-and-lens-laws | all 30 valid cases satisfy all five preservation laws |
