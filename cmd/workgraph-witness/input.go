...
func loadInput(value options) (workgraph.Contract, workgraph.Observation, error) {
	...
	if err != nil {
		return workgraph.Contract{}, workgraph.Observation{}, err
	}
	var contract workgraph.Contract
	if err := json.Unmarshal(contractData, &contract); err != nil {
		return contract, workgraph.Observation{}, err
	}
	...
	if observation.SourcePath != contract.Source {
		return contract, observation, fmt.Errorf("source path does not match contract authority")
	}
