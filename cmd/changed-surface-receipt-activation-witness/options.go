...
type options struct {
	assurance, eligibility string
	subjectSHA, output     string
}

func parseOptions(args []string, stderr io.Writer) (options, error) {
	var opts options
	set := flag.NewFlagSet("changed-surface-receipt-activation-witness", flag.ContinueOnError)
	...
	set.StringVar(&opts.subjectSHA, "subject-sha", "", "exact activation candidate SHA")
	...
	if opts.assurance == "" || opts.eligibility == "" || opts.subjectSHA == "" || opts.output == "" {
		return options{}, fmt.Errorf("--assurance, --eligibility, --subject-sha, and --out are required")
	}
	return opts, nil
}
