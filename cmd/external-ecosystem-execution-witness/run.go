...
func run(args []string) int {
	...
	if o.mode == "observe" {
		err = writeJSON(o.observation, observation)
	}
	if err == nil {
		err = writeJSON(o.report, report)
	}
	if err == nil {
		err = writeJSON(o.suite, suite)
	}
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 2
	}
	if report.Decision != execution.DecisionConfirmed || report.Resolution != "EXACT" || suite.Passed != suite.Total {
		return 1
	}
	return 0
}
