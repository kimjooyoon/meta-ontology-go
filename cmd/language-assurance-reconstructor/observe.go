...
func decide(observation rawObservation) (string, string, string) {
	...
	if observation.UnknownTopDecisions != nil && *observation.UnknownTopDecisions > 0 {
		return failClosed, reasonUnknown, invariantOnly
	}
	if *observation.ExactSnapshotBindingBPS < 10000 {
		return block, reasonSnapshot, exact
	}
	if *observation.SelfMintingPaths > 0 || *observation.RoleConflictPaths > 0 || *observation.UnknownLaunderingPaths > 0 {
		return block, reasonGovernance, exact
	}
