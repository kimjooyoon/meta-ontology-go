...
func main() {
	...
	flag.Parse()
	if *input == "" || *output == "" {
		fail(entityfields.FailureReport("ENTITY_FIELDS_INPUT_ARGUMENT_MISSING"), *output)
		os.Exit(1)
	}
	source, err := os.ReadFile(*input)
	if err != nil {
		fail(entityfields.FailureReport("ENTITY_FIELDS_SOURCE_MISSING"), *output)
		os.Exit(1)
	}
	observation, err := entityfieldsv1.Observe(filepath.ToSlash(*input), string(source))
	if err != nil {
		fail(entityfields.FailureReport(err.Error()), *output)
		os.Exit(1)
	}
	report := entityfields.Evaluate(observation)
	if err := entityfields.Verify(observation, report); err != nil {
		fail(entityfields.FailureReport(err.Error()), *output)
		os.Exit(1)
	}
	if err := writeSuccess(*output, observation, report); err != nil {
		fail(entityfields.FailureReport(err.Error()), *output)
		os.Exit(1)
	}
}
...
func writeSuccess(root string, observation entityfieldsv1.Observation, report entityfields.Report) error {
	...
	files := map[string][]byte{
		...
		"lsp.json": mustJSON(struct {
			Symbols    []entityfieldsv1.NavigationSymbol    `json:"symbols"`
			References []entityfieldsv1.NavigationReference `json:"references"`
		}{observation.Symbols, observation.References}),
...
func mustJSON(value any) []byte {
	...
	if err != nil {
		return []byte(`{"decision":"REFUTED","reason":"ENTITY_FIELDS_JSON_ENCODING_FAILED"}`)
	}
	return append(data, '\n')
}
