...
func run(cfg config, stdout io.Writer) error {
	if cfg.root == "" || cfg.subjectSHA == "" || cfg.input == "" || cfg.output == "" {
		return fmt.Errorf("root, subject-sha, input, and output are required")
	}
	...
	if err := writeReceipt(cfg.output, receipt); err != nil {
		return err
	}
