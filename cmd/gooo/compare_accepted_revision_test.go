...
func TestCompareAcceptedRevisionRejectsUnverifiedCounterexampleRecovery(t *testing.T) {
	root := t.TempDir()
...
`)
	...
	if code := runAcceptedRevision(args, OSFileReader{}, &stdout, &stderr); code == exitOK {
		t.Fatalf("unverified counterexample recovery was adopted: stdout=%q", stdout.String())
	}
}
