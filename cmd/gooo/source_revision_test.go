...
`)
	if err := os.WriteFile(baselinePath, baseline, 0o644); err != nil { t.Fatal(err) }
	if err := os.WriteFile(inputPath, []byte(`{"value":9223372036854775807}`), 0o644); err != nil { t.Fatal(err) }
	...
	var decoded struct { State string `json:"state"`; Accepted bool `json:"accepted"` }
	if err := json.Unmarshal(report, &decoded); err != nil { t.Fatal(err) }
	if decoded.State != "CLOSED" || !decoded.Accepted { t.Fatalf("evaluation = %#v", decoded) }
}
