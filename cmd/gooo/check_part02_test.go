...
func TestRunDispatchesCheckAndUsage(t *testing.T) {
	...
	stdout.Reset()
	stderr.Reset()
	if code := run(nil, &stdout, &stderr); code != exitUsage || stderr.String() != "usage: gooo <run|profile|debug|test|emit|check|generate|roundtrip|query|inspect|graph|analyze|format|fix|provenance|selective-ci|lsp|version> [args]\n" {
