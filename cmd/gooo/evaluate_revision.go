...
func runEvaluateRevision(args []string, reader SourceReader, stdout, stderr io.Writer) int {
	...
	if err != nil { fmt.Fprintln(stderr, err); return exitFailure }
	...
	if err := json.Unmarshal(revisionData, &revision); err != nil { fmt.Fprintf(stderr, "gooo: evaluate-revision: decode revision: %v\n", err); return exitFailure }
	inputData, err := reader.ReadFile(options.input)
	if err != nil { fmt.Fprintln(stderr, err); return exitFailure }
	input, err := decodePlanInput(inputData)
	if err != nil { fmt.Fprintf(stderr, "gooo: evaluate-revision: input: %v\n", err); return exitFailure }
	...
	if err != nil { fmt.Fprintf(stderr, "gooo: evaluate-revision: encode: %v\n", err); return exitFailure }
	payload = append(payload, '\n')
	if err := writeRevisionEvaluation(options.outputDir, payload); err != nil { fmt.Fprintf(stderr, "gooo: evaluate-revision: output: %v\n", err); return exitFailure }
	fmt.Fprintf(stdout, "source revision evaluation: %s\n", filepath.Join(options.outputDir, "evaluation.json"))
	if evaluation.State == valueexecution.ReplayClosed { return exitOK }
	return exitFailure
}
...
func parseEvaluateRevisionArguments(args []string) (evaluateRevisionOptions, error) {
	...
	for index := 0; index < len(args); index++ {
		...
		if index+1 >= len(args) || seen[args[index]] { return evaluateRevisionOptions{}, fmt.Errorf("%s", evaluateRevisionUsage) }
		seen[args[index]] = true
		index++
		switch args[index-1] {
		case "--revision": options.revision = args[index]
		case "--activity": options.activity = args[index]
		case "--input": options.input = args[index]
		case "--out": options.outputDir = args[index]
		default: return evaluateRevisionOptions{}, fmt.Errorf("%s", evaluateRevisionUsage)
		}
	}
...
func writeRevisionEvaluation(outputDir string, payload []byte) error {
	...
	if len(entries) != 0 { return fmt.Errorf("caller-owned output directory must be empty") }
