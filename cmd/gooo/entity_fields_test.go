...
func TestEntityFieldsProjectionPreservesBidirIdentityOrderSpansAndProfile(t *testing.T) {
	...
	for index, got := range projected.Entities[0].Fields {
		...
		wantTypeID := want.TypeRef.ID
		if wantTypeID == "" {
			wantTypeID = semantic.ID(want.TypeRefUse.ResolvedID)
		}
		if got.ID != string(want.ID) || got.Parent != string(want.Parent) || got.Name != want.Name || got.TypeRefID != string(wantTypeID) || got.Presence != string(want.Presence) || got.Cardinality != string(want.Cardinality) {
