...
type consumerReport struct {
	...
	SyntheticEvidenceCount                int                   `json:"synthetic_evidence_count"`
...
func consume(policyPath, casesPath, artifactDir, outputPath string) error {
	...
	imports, err := observeImportBoundary(judge)
	if err != nil {
		return err
	}
	report := consumerReport{
		...
		ImportBoundary:    imports,
		SyntheticEvidence: make([]evidenceObservation, 0, len(cases)), CurrentEvidence: current,
		SyntheticEvidenceCount: len(cases), CurrentEvidenceCount: 1,
		SubjectResolution: subjectUnresolved, SubjectDecision: currentDecision, IndependentResults: reconstructed,
	}
	for _, value := range cases {
		report.SyntheticEvidence = append(report.SyntheticEvidence, evidenceObservation{Class: value.EvidenceClass, CaseID: value.ID, ProducerAvailable: value.ProducerAvailable, ConsumerAvailable: value.ConsumerAvailable, SourceDigest: value.ObservedSourceDigest, ArtifactSourceDigest: value.ObservedArtifactSourceDigest, ArtifactDigest: digestBytes(artifactBytes), GeneratedJudgeDigest: value.ObservedGeneratedJudgeDigest, IndependentDigest: value.ObservedIndependentDigest, IndependentReconstructionDigest: compiled.SemanticDigest, SemanticDigest: compiled.SemanticDigest, ObservationDigest: digestInput(value), Provenance: value.Provenance})
	}
	if currentDecision.Decision == decisionPass {
...
func observeCurrentEvidence(dir string, policy policy, source []byte, artifact artifact, judge []byte) evidenceObservation {
	...
	observation := evidenceObservation{Class: evidenceCurrent, CaseID: "current-subject", ProducerAvailable: available, ConsumerAvailable: available, SourceDigest: digestBytes(source), ArtifactSourceDigest: artifact.Policy.SourceDigest, ArtifactDigest: artifactDigest, GeneratedJudgeDigest: digestBytes(judge), IndependentDigest: policy.SemanticDigest, IndependentReconstructionDigest: policy.SemanticDigest, SemanticDigest: policy.SemanticDigest, Provenance: provenance}
	observation.ObservationDigest = digestEvidence(observation)
	return observation
}

func observeImportBoundary(judge []byte) (importBoundary, error) {
	_, consumerSourcePath, _, ok := runtime.Caller(0)
	if !ok {
		return importBoundary{}, errors.New("consumer source path is unavailable")
	}
	consumerSource, err := os.ReadFile(consumerSourcePath)
	if err != nil {
		return importBoundary{}, fmt.Errorf("read consumer source boundary: %w", err)
	}
	consumerForbidden, err := countProducerImports(consumerSource, consumerSourcePath)
	if err != nil {
		return importBoundary{}, err
	}
	generatedForbidden, err := countProducerImports(judge, "generated-judge.go")
	if err != nil {
		return importBoundary{}, err
	}
	return importBoundary{
		ProducerCompilerImports:     fmt.Sprintf("%d/1", consumerForbidden),
		GeneratedTemplateImports:    fmt.Sprintf("%d/1", generatedForbidden),
		IndependentEvaluatorImports: fmt.Sprintf("%d/1", consumerForbidden),
	}, nil
}

func countProducerImports(source []byte, filename string) (int, error) {
