...
func produce(policyPath, casesPath, outputDir string) error {
	...
	if independentProgram.SourceDigest() != policy.SourceDigest || independentProgram.SemanticDigest() != policy.SemanticDigest {
		return errors.New("independent raw-source contract digest differs from producer compilation")
	}
	...
	generated, independentResults, err := executeAll(judge, independentProgram, cases)
	if err != nil {
		return err
	}
	receipt, err := policycompilation.BuildReceipt(policy, artifact, judgeHash, cases, generated, independentResults, policycompilation.WriteSetObservation{RepositoryBeforeDigest: beforeDigest, RepositoryBeforeCount: beforeCount, GeneratedRootClass: "RUNNER_TEMP_ONLY", GeneratedFiles: []string{"artifact.json", "generated-results.json", "independent-results.json", "judge.go", "policy.json", "receipt.json"}})
	if err != nil {
		return err
	}
	...
	if err := writeJSON(filepath.Join(outputDir, "independent-results.json"), independentResults); err != nil {
		return err
	}
...
func executeAll(judge []byte, program independent.Program, cases []policycompilation.Case) ([]policycompilation.DecisionResult, []policycompilation.DecisionResult, error) {
	...
	for _, input := range cases {
		...
		replay := program.Evaluate(independentInput)
		independent = append(independent, policycompilation.DecisionResult{
			CaseID: replay.CaseID, Decision: replay.Decision, Stage: replay.Stage, Step: replay.Step, Reason: replay.Reason,
			PolicyDigest: replay.PolicyDigest, SemanticDigest: replay.SemanticDigest, Denominator: replay.Denominator,
		})
	}
