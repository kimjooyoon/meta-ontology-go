...
type options struct {
	before, semanticAfter, commentAfter, corpus, out string
}
...
func parseOptions(args []string) (options, bool) {
	...
	valid := flags.Parse(args) == nil && value.before != "" && value.semanticAfter != "" && value.commentAfter != "" && value.corpus != "" && value.out != ""
	return value, valid
}
