...
func run(cfg config, stdout io.Writer) error {
	if cfg.root == "" || cfg.corpus == "" || cfg.concept == "" || cfg.expectedHead == "" {
		return fmt.Errorf("root, corpus, concept, and expected-head are required")
	}
	...
	if cfg.check != "" {
		...
		if err := metalsp.Validate(existing, cfg.expectedHead); err != nil {
			return err
		}
		if !reflect.DeepEqual(existing, report) {
			return fmt.Errorf("toolchain lsp replay mismatch")
		}
	} else if err := writeExclusive(cfg.output, report); err != nil {
		return err
	}
	...
	if report.Decision != metalsp.DecisionPass {
