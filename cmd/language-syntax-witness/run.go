...
func run(cfg config, stdout io.Writer) error {
	...
	for _, external := range []string{cfg.concept, target} {
		...
		if err != nil || !outside {
			return fmt.Errorf("language syntax artifacts must be outside the repository root")
		}
	}
	if cfg.check != "" {
...
func build(cfg config) (languagesyntax.Report, error) {
	registryRaw, err := os.ReadFile(cfg.registry)
	if err != nil {
		return languagesyntax.Report{}, err
	}
	...
	report := languagesyntax.Evaluate(os.DirFS(cfg.root), cfg.head, registryRaw, artifact)
