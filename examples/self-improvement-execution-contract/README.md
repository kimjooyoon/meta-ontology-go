...
contract. The live request must carry the first-class value-witness
execution-input artifact; the contract does not substitute a synthetic source
path, activity, or input case.
...
`CLOSED/DECLARED` result means only that the twelve pre-execution fields and the
source-snapshot-backed execution input are exact and bound to the fixed
operation registry. Execution remains blocked by a
