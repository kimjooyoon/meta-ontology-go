...
LOWER_RESOLUTION`. An `UNKNOWN` input remains `UNKNOWN / LOWER_RESOLUTION`
and retains its stage, step, reason, and `OPEN` claim transition.
...
case/intervention IDs, and the fixed denominator of `2`. The independent
...
and counts. Both are replayed in memory with zero repository writes.
